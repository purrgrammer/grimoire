...
function getContentDisplay(event: NostrEvent): ContentDisplay {
  ...
  if (encryption && !isAppDataUnlocked(event))
    return { kind: "locked", method: encryption };
    ...
    <>
      <div className="navbar border-b border-base-300 gap-2">
        <div className="flex-1 min-w-0 px-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {encryption && (
              <span className="badge badge-warning">{encryption}</span>
            )}
          </div>
          <div className="opacity-60 truncate">
            {new Date(event.created_at * 1000).toLocaleString()} ·{" "}
            {event.content.length} chars · {event.tags.length} tags
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div>
          ...
          {display?.kind === "empty" && (
            <p className="opacity-60">This event has no content.</p>
          )}
          {display?.kind === "locked" && (
          ...
          {display?.kind === "raw" && (
            <>
              <p className="opacity-60 mb-2">
                ...
                {isBinary(display.value)
                  ? "binary data"
                  : "non-JSON text"} · {display.value.length} chars
              </p>
...
function EventEditor({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const signed = await AppDataFactory.modify(event)
        .as(signer)
        .data(parsed, encryption)
        .sign();
      onSave(signed);
    } catch (err) {
        ...
        <div className="flex gap-2">
          <select
            ...
            onChange={(e) =>
              setEncryption(
                e.target.value
                  ? (e.target.value as EncryptionMethod)
                  : undefined,
              )
            }
            disabled={saving}
...
function AppDataManager({ user }: { user: User }) {
  ...
  const events = use$(
    () =>
      eventStore.timeline({ kinds: [APP_DATA_KIND], authors: [user.pubkey] }),
    [user.pubkey],
  );
                    ...
                    >
                      <div className="flex flex-col gap-1 min-w-0 w-full">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {enc && (
                            <span className="badge badge-warning">{enc}</span>
                          )}
                        </div>
