    ...
    >
      <div className="w-48 min-w-48 h-full shrink-0 overflow-hidden bg-base-200">
        {article.image ? (
          ...
          <div className="size-full flex items-center justify-center text-xs text-base-content/40">
            No image
          </div>
        )}
      </div>
      <div className="flex flex-col gap-1 p-4 overflow-hidden">
        ...
        <p className="text-sm opacity-70">
          By {profile?.displayName || author.npub.slice(0, 8)} •{" "}
...
function CommentsSection({ article }: { article: Article }) {
  const comments = use$(
    () =>
      eventStore
        .model(CommentsModel, article.event)
  ...
  use$(() => {
    ...
    return pool
      ...
      .subscription({
        kinds: [1111],
        "#a": [
          `30023:${selected.event.pubkey}:${selected.event.tags.find((t) => t[0] === "d")?.[1] || ""}`,
        ],
      })
  ...
  return (
    <div className="container mx-auto max-w-6xl p-4">
      <div className="py-8">
        ...
        {relay && articles && (
          <ArticleList articles={articles} onArticleSelect={setSelected} />
        )}
      </div>
    </div>
  );
