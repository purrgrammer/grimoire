...
function PinnedBadgeCard({
  ...
  return (
    ...
    >
      <span className="text-base-content/40 font-mono text-sm w-6 text-center select-none">
        {index + 1}
      </span>
...
function ResolvedPinnedSlot(props: {
  ...
  const resolved = use$(
    () => props.profileBadges.slot$(props.index),
    [props.profileBadges, props.index],
  );
  return (
    <PinnedBadgeCard
    ...
    <div className="flex items-center gap-3 p-3 border-b border-base-200 last:border-b-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-base-content/40">
          Issued by {formatPubkey(award.issuer.pubkey)} ·{" "}
          {award.createdAt.toLocaleDateString()}
        </div>
      </div>
      <button
      ...
      <div className="modal-box max-w-lg">
        ...
        {awards.length === 0 ? (
          <div className="text-center text-base-content/50 py-8">
            No badge awards found for your account.
          </div>
        ) : (
...
function getFactory(event: NostrEvent | undefined) {
  return event
    ? ProfileBadgesFactory.modify(event)
    : ProfileBadgesFactory.create();
}
...
function EditorView({ user }: { user: User }) {
  ...
  const runAction = useCallback(
    async (action: string, factory: ProfileBadgesFactory) => {
      ...
      try {
        ...
        const relays = mailboxesRef.current
          ? relaySet(RELAYS, mailboxesRef.current.outboxes)
          : RELAYS;
  ...
  const onDragStart = useCallback(
    (i: number) => setDragState({ dragging: i, over: null }),
    [],
  );
  const onDragOver = useCallback(
    (i: number) => setDragState((s) => ({ ...s, over: i })),
    [],
  );
  ...
  const handleRemove = useCallback(
    (index: number) => {
      ...
      runAction(
        "remove",
        getFactory(badgesRef.current?.event).removeByBadge(slot.badge),
      );
    },
    [runAction],
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-base-content/60">
            Drag to reorder, add from awards, or remove pinned badges.
          </p>
        </div>
        ...
        <div className="space-y-1">
          {slots.map((slot, i) =>
            profileBadges ? (
              <ResolvedPinnedSlot
                key={
                  getReplaceableAddressFromPointer(slot.badge) +
                  ":" +
                  slot.award.id
                }
                slot={slot}
              ...
              <div
                key={i}
                className="flex items-center gap-3 p-3 border border-base-300 rounded-lg animate-pulse"
              >
              ...
              >
                ...
                <div className="space-y-1">
                  {entry.results.map((r) => (
                    ...
                    >
                      <span
                        className={`badge badge-xs ${r.ok ? "badge-success" : "badge-error"}`}
                      >
                        {r.ok ? "OK" : "FAIL"}
                      </span>
