...
function RecipientAvatar({ user }: { user: User }) {
  ...
  const name =
    profile?.displayName || profile?.name || formatPubkey(user.pubkey);
  ...
  return (
    <div
      className="avatar tooltip cursor-pointer"
      data-tip={name}
      onClick={handleClick}
    >
...
function ModalAwardRow({ award, index }: { award: BadgeAward; index: number }) {
  ...
  return (
    <li>
      ...
      <div
        className={
          isEven ? "timeline-start mb-10 md:text-end" : "timeline-end md:mb-10"
        }
      >
        <time className="font-mono italic text-xs">
          {award.createdAt.toLocaleString()}
        </time>
        <div className="text-sm">
          Awarded to {award.recipients.length} recipient
          {award.recipients.length === 1 ? "" : "s"}
        </div>
      </div>
    </li>
  );
...
function BadgeDetailsModal() {
  ...
  const pointer = useMemo(
    () => (badge ? getAddressPointerForEvent(badge.event) : undefined),
    [badge],
  );
      ...
      [
        {
          ...
          "#a": [getReplaceableAddressFromPointer(pointer)],
        },
      ],
      { eventStore },
    );
      ...
      <div className="modal-box max-w-2xl">
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {badge.description && (
              <p className="text-base-content/80 mt-2">{badge.description}</p>
            )}
          </div>
        </div>
        ...
        {awards && awards.length === 0 && (
          <div className="text-center text-base-content/50 py-4">
            No award events found for this badge.
          </div>
        )}
      </div>
...
function BadgeSlotCard({
  ...
  const image = badge.image?.url;
  const isRecipient =
    award?.recipients.some((p) => p.pubkey === user.pubkey) ?? false;
            ...
            <div>
              ...
              <span className="font-medium">
                {issuerProfile?.displayName ||
                  formatPubkey(award?.issuer.pubkey ?? "")}
              </span>
            </div>
            <div className="text-xs">
              {award.recipients.length} recipient tag
              {award.recipients.length === 1 ? "" : "s"}
            </div>
          </div>
        ) : (
          <p className="text-sm text-base-content/50">
            Waiting for award details…
          </p>
        )}
      </div>
    </div>
...
function AwardRow({
  ...
  const name =
    badge?.name ||
    badge?.identifier ||
    `${badge?.event.kind}:${badge?.identifier}`;
  const image = badge?.image?.url;
  ...
  const content = (
    <>
      ...
      <div className="text-sm text-base-content/70">
        {award.recipients.length} recipient
        {award.recipients.length === 1 ? "" : "s"}
      </div>
    </>
  );
...
export default function ProfileBadgesExample() {
  ...
  use$(() => {
    ...
    return pool.subscription(
      user?.mailboxes$.pipe(
        map((all) => relaySet(RELAYS, all?.inboxes, all?.outboxes)),
      ),
      [
        {
          kinds: [kinds.Metadata, PROFILE_BADGES_KIND],
          authors: [user.pubkey],
        },
        {
          kinds: [LEGACY_PROFILE_BADGES_KIND],
          authors: [user.pubkey],
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
          ...
          <div className="card-body p-4 gap-1">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="card-title">
                {profile.displayName ||
                  profile.name ||
                  formatPubkey(profile.event.pubkey)}
              </h2>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {profileBadges && (
            <span className="badge badge-neutral">{profileBadges.count}</span>
          )}
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {awards && (
            <span className="badge badge-neutral">{awards.length}</span>
          )}
        </div>
        {pubkey && !awards && (
          <div className="flex justify-center py-4">
        ...
        {awards && awards.length === 0 && (
          <div className="alert">
            <span>
              No badge award events referencing this pubkey were found during
              this session.
            </span>
          </div>
        )}
