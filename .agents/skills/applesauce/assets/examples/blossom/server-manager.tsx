...
function ServerItem({
  ...
  return (
    <div
      className={`flex items-center gap-3 p-2 hover:bg-base-100 rounded ${isDefault ? "bg-primary/10" : ""}`}
    >
      {/* Favicon */}
...
export default function BlossomServerManager() {
  ...
  const servers = use$(
    () =>
      pubkey
        ? eventStore.blossomServers(
            addRelayHintsToPointer({ pubkey, relays: [] }, mailboxes?.outboxes),
          )
        : undefined,
    [pubkey, mailboxes],
  );
    ...
    async (url: string) => {
      ...
      try {
        ...
        const events = await firstValueFrom(
          actions.exec(AddBlossomServer, url).pipe(toArray()),
        );
  ...
  const handleRemoveServer = useCallback(
    async (server: URL) => {
      ...
      try {
        setError(null);
        const events = await firstValueFrom(
          actions.exec(RemoveBlossomServer, server).pipe(toArray()),
        );
  ...
  const handleSetDefault = useCallback(
    async (server: URL) => {
      ...
      try {
        setError(null);
        const events = await firstValueFrom(
          actions.exec(SetDefaultBlossomServer, server).pipe(toArray()),
        );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-base-content/70">
            Manage your Blossom media servers. The first server in the list is
            your default server.
          </p>
        </div>
        {pubkey ? (
      ...
      {!pubkey ? (
        <div className="text-center py-20">
          <h2 className="text-2xl font-bold mb-4">
            Welcome to Blossom Server Manager
          </h2>
        ...
        <div className="space-y-6">
          ...
          <div>
            <h2 className="text-lg font-semibold mb-4">
              Your Blossom Servers ({servers?.length || 0})
            </h2>
