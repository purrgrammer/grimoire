...
function BookmarkEventForm({
  onAdd,
}: {
  onAdd: (event: NostrEvent, hidden: boolean) => Promise<void>;
}) {
...
function BookmarkManager({ user }: { user: User }) {
  ...
  const hiddenNotes = useMemo(
    () => hidden?.filter((pointer) => isEventPointer(pointer)),
    [hidden],
  );
  const hiddenArticles = useMemo(
    () => hidden?.filter((pointer) => isAddressPointer(pointer)),
    [hidden],
  );
  ...
  const handleAdd = async (event: NostrEvent, hidden: boolean) => {
    ...
    try {
      await actions
        .exec(BookmarkEvent, event, undefined, hidden)
        .forEach(async (signed) => {
  ...
  const handleRemove = async (
    event: EventPointer | AddressPointer,
    hidden: boolean,
  ) => {
    if (!signer || !outboxes?.length) return;
    ...
    try {
      await actions
        .exec(UnbookmarkEvent, event, undefined, hidden)
        .forEach(async (signed) => {
          eventStore.add(signed);
          await pool.publish(outboxes, signed);
        });
    ...
    try {
      ...
      alert(
        err instanceof Error
          ? err.message
          : "Failed to unlock hidden bookmarks",
      );
    } finally {
      setUnlocking(false);
    }
    ...
    <div className="container mx-auto my-8 px-4 max-w-4xl">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm opacity-70">
                  Unlock to view hidden bookmarks
                </p>
              </div>
              <button
                className="btn btn-primary"
      ...
      <div className="space-y-2">
        {hidden ? (
          hidden.length === 0 ? (
            <div className="alert alert-info">
              <span>
                No bookmarks yet. Add a bookmark using the form above.
              </span>
            </div>
          ) : (
        ...
        {hiddenArticles && hiddenArticles.length > 0 && (
          <div>
            <h2 className="text-xl font-bold mb-3">
              Articles ({hiddenArticles.length})
            </h2>
            <div className="space-y-2">
