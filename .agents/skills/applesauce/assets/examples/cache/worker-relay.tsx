...
persistEventsToCache(eventStore, (events) =>
  Promise.allSettled(events.map((event) => workerRelay.event(event))).then(
    (results) => {
...
function ReplyLine({ reply }: { reply: Note }) {
  ...
  return (
    <div>
      <div>
        Relying to{" "}
        {profile?.displayName || `${reply.author.npub.slice(0, 8)}...`}:
      </div>
...
function NoteCard({ note }: { note: Note }) {
  ...
  const reply = use$(
    () => note.replyingTo$.pipe(castEventStream(Note, note.store)),
    [note.id],
  );
...
function CacheStats() {
  ...
  const kinds =
    summary?.kinds && typeof summary.kinds === "object"
      ? (summary.kinds as Record<string, number>)
      : null;
...
export default function WorkerRelayCacheExample() {
  ...
  const notes = use$(
    () =>
      eventStore
        .timeline({ kinds: [1] })
        .pipe(debounceTime(500), castTimelineStream(Note)),
    [],
  );
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-base-content/70">
          Cache Nostr events using Worker Relay SQLite with full-text search and
          standard Nostr filters.
        </p>
      </div>
