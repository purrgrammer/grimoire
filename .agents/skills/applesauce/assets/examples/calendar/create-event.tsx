 ...
 */
import {
  DateBasedCalendarEventFactory,
  TimeBasedCalendarEventFactory,
...
function buildDateBasedCalendarEvent(
  data: CalendarEventFormData,
): DateBasedCalendarEventFactory {
  let factory = DateBasedCalendarEventFactory.create(data.title).description(
    data.description,
  );
  ...
  for (const loc of data.locations.filter((l) => l.value.trim()))
    factory = factory.location(loc.value.trim());
  for (const link of data.referenceLinks.filter((l) => l.value.trim()))
    factory = factory.referenceLink(link.value.trim());
  return factory;
}

function buildTimeBasedCalendarEvent(
  data: CalendarEventFormData,
): TimeBasedCalendarEventFactory {
  let factory = TimeBasedCalendarEventFactory.create(data.title).description(
    data.description,
  );
  if (data.summary) factory = factory.summary(data.summary);
  if (data.image) factory = factory.image(data.image);
  if (data.startDateTime)
    factory = factory.startTime(new Date(data.startDateTime), data.timezone);
  if (data.endDateTime)
    factory = factory.endTime(new Date(data.endDateTime), data.timezone);
  if (data.geohash) factory = factory.geohash(data.geohash);
  for (const loc of data.locations.filter((l) => l.value.trim()))
    factory = factory.location(loc.value.trim());
  for (const link of data.referenceLinks.filter((l) => l.value.trim()))
  ...
  const onSubmit = async (data: CalendarEventFormData) => {
    try {
      ...
      if (typeof window === "undefined" || !(window as any).nostr) {
        ...
        setError(
          "No Nostr extension found. Please install a Nostr browser extension like Alby or nos2x.",
        );
        return;
      }
      ...
      const builder =
        data.eventType === "date"
          ? buildDateBasedCalendarEvent(data)
          : buildTimeBasedCalendarEvent(data);
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold mb-2">
          NIP-52 Calendar Event Creator
        </h1>
        <p className="text-lg text-base-content/70">
          Create date-based or time-based calendar events following the NIP-52
          specification
        </p>
      </div>
      ...
      {error && (
        <div
          className={`alert mb-6 ${noSignerWarning ? "alert-warning" : "alert-error"}`}
        >
          {error}
          {noSignerWarning && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            >
              {(field) => (
                <TextInput field={field} placeholder="Brief summary" />
              )}
            </FormField>
          ...
          <div className="space-y-6">
            ...
            {eventType === "date" ? (
              <>
                <FormField
                  ...
                  required
                  rules={{
                    required: "Start date is required for date-based events",
                  }}
                >
                  {(field) => (
                    <TextInput
                ...
                >
                  {(field) => (
                    <TextInput field={field} type="date" min={startDate} />
                  )}
                </FormField>
              </>
            ) : (
              <>
                <FormField
                  ...
                  required
                  rules={{
                    required: "Start time is required for time-based events",
                  }}
                >
                  {(field) => (
                    <TextInput
                ...
                >
                  {(field) => (
                    <TextInput field={field} placeholder="America/New_York" />
                  )}
                </FormField>
              </>
            )}
            ...
            >
              {(field) => (
                <TextInput field={field} placeholder="9q8yyk8yuzvw" />
              )}
            </FormField>
          </div>
        </div>
