...
function CalendarEventCard({
  ...
  // Load RSVPs count
  const rsvps = use$(
    () => eventStore.model(CalendarEventRSVPsModel, event),
    [event.id],
  );
        ...
        <div className="space-y-2">
          {start && (
            <div className="flex items-center gap-2 text-sm">
              <span className="font-medium">
                {isUpcoming ? "Starts:" : isPast ? "Started:" : "Time:"}
              </span>
          ...
          {locations.length > 0 && (
            <div className="flex items-center gap-2 text-sm">
              ...
              {locations.length > 1 && (
                <span className="text-xs">+{locations.length - 1} more</span>
              )}
            </div>
          )}
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex items-center gap-2">
              {rsvpCount > 0 && (
                <span className="badge badge-secondary">{rsvpCount} RSVPs</span>
              )}
              <span
                className={`badge ${isUpcoming ? "badge-success" : isPast ? "badge-warning" : "badge-neutral"}`}
              >
                  ...
                  {rsvps.map((rsvp) => (
                    ...
                    >
                      <Avatar
                        user={{
                          pubkey: rsvp.pubkey,
                          relays: Array.from(getSeenRelays(rsvp) || []),
                        }}
                      />
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-base-content/70">
                          RSVP •{" "}
                          {new Date(
                            rsvp.created_at * 1000,
                          ).toLocaleDateString()}
                        </div>
                      </div>
                      {rsvp.content && (
                        <div className="text-sm bg-base-100 px-2 py-1 rounded max-w-xs">
                          {rsvp.content}
                        </div>
                      )}
                    </div>
