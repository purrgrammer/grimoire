...
function RepositoryCard({ repo }: { repo: Repository }) {
  ...
  const picture =
    profile?.picture || `https://robohash.org/${repo.author.pubkey}.png`;
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-base-content/60">
            {repo.author.npub.slice(0, 20)}...
          </p>
        </div>
        <time className="text-sm text-base-content/60">
          {repo.createdAt.toLocaleDateString()}
        </time>
      </div>
        ...
        {repo.clone.length > 0 && (
          <div className="text-sm mb-2">
            ...
            <button
              ...
              onClick={() => navigator.clipboard.writeText(repo.clone[0])}
            >
              Copy
            </button>
          </div>
