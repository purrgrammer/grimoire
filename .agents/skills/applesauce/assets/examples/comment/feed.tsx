...
const appRelays = new BehaviorSubject<string[]>([
  "wss://relay.damus.io/",
  "wss://nos.lol/",
...
function CommentItem({ comment }: { comment: Comment }) {
  ...
  const picture =
    profile?.picture || `https://robohash.org/${comment.author.pubkey}.png`;
          ...
          {parentEvent.kind === COMMENT_KIND ? (
            <>
              <span className="font-medium">
                {parentProfile?.displayName || parentEvent.pubkey.slice(0, 8)}
                ...
              </span>
              {": "}
              ...
              {" - "}
              <time>
                {new Date(parentEvent.created_at * 1000).toLocaleDateString()}
              </time>
            </>
          ) : (
...
function ReplyForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const signed = await CommentFactory.create(
        parentEvent,
        content.trim(),
      ).sign(signer);
...
function ThreadComment({
  ...
  return (
    <>
      <div
        className={`${depth > 0 ? "ml-6 mt-4 border-l-2 border-base-300 pl-4" : ""}`}
      >
      ...
      .subscription(appRelays, {
        ...
        ...(rootPointer.type === "event"
          ...
          : // Root is a replaceable event
            {
              "#a": [
                `${rootPointer.kind}:${rootPointer.pubkey}:${rootPointer.identifier}`,
              ],
            }),
        limit: 500,
      })
...
export default function CommentFeed() {
  ...
  if (selectedThread)
    return (
      <ThreadView
        rootPointer={selectedThread}
        onBack={() => selectedThread$.next(null)}
      />
    );
    ...
    <div className="max-w-6xl w-full mx-auto my-8 px-4">
      <div className="mb-6">
        ...
        <p className="text-base-content/60 mb-4">
          A feed of NIP-22 kind 1111 comments. Click on any comment to view the
          full thread.
        </p>
        ...
        {comments === undefined ? (
          ...
          <div className="text-center py-12">
            <p className="text-base-content/60">
              No comments found. Try a different relay.
            </p>
          </div>
        ) : (
