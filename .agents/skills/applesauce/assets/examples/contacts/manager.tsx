...
function ContactManager({ user }: { user: User }) {
  ...
  const handleAdd = useCallback(
    async (pubkey: string) => {
      if (!outboxes?.length) {
        throw new Error(
          "No outbox relays available. Please set up your mailboxes (NIP-65).",
        );
      }
  ...
  const handleUnfollow = useCallback(
    async (contact: User) => {
      ...
      try {
        await actions
          .exec(UnfollowUser, contact.pubkey)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {contacts.map((contact) => (
            <ContactCard
              key={contact.pubkey}
              user={contact}
              onUnfollow={() => handleUnfollow(contact)}
            />
          ))}
        </div>
      ) : (
