...
class WorkerRelayEventDatabase implements IAsyncEventDatabase {
  ...
  async hasReplaceable(
    kind: number,
    pubkey: string,
    identifier?: string,
  ): Promise<boolean> {
    return (
      (await this.relay.count([
        "REQ",
        pubkey,
          ...
          <li key={event.id} className="list-row relative">
            ...
            <div>
              <div className="font-bold text-md">
                {getDisplayName(profile, event.pubkey.slice(0, 8) + "...")}
              </div>
              <div className="text-xs uppercase font-semibold opacity-60">
                {profile.nip05}
              </div>
            </div>
            <p className="list-col-wrap text-xs whitespace-pre truncate max-h-48">
              {profile.about}
            </p>

            <button
              className="btn btn-soft btn-primary absolute top-2 right-2"
              onClick={() => viewEvent$.next(event)}
            >
              View
            </button>
      ...
      <div className="card-body">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <div className="font-semibold">
              {getDisplayName(profile) || `${note.pubkey.slice(0, 8)}...`}
            </div>
        ...
        <div className="flex justify-between items-center mt-4 pt-3 border-t border-base-300">
          ...
          <div className="text-xs text-base-content/50">
            {note.tags.length > 0 && `${note.tags.length} tags`}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-3xl font-bold">
            Searchable browser SQLite event database with import/export
          </h1>
        ...
        <p className="text-base-content/70">
          Import, export, search, and filter Nostr events using Worker Relay
          SQLite with full-text search and standard Nostr filters.
        </p>
      </div>
      ...
      {importStats && (
        <div className="alert alert-success mb-6">
          <div>
            ...
            <p>
              Added: {importStats.added} | Failed: {importStats.failed} | Total:{" "}
              {importStats.total}
            </p>
          </div>
        </div>
        ...
        >
          <div>
            ...
            >
              ...
              <option value="30019">
                Create or update a pickup method (30019)
              </option>
      ...
      <div className="mb-6">
        ...
        {!isLoading && !searchQuery && searchResults.length === 0 && !error && (
          <div className="text-center text-base-content/70 mt-8">
            No events in database, import some
          </div>
        )}
      </div>
      ...
      <dialog className={`modal ${viewEvent ? "modal-open" : ""}`}>
        <div className="modal-box">
          <pre className="text-xs overflow-auto font-mono">
            {JSON.stringify(viewEvent, null, 2)}
          </pre>
        </div>
