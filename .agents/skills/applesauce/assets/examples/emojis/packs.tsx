...
function PackAuthor({
  ...
  const displayName = getDisplayName(
    profile,
    pack.author.pubkey.slice(0, 8) + "...",
  );
  const picture = getProfilePicture(
    profile,
    `https://robohash.org/${pack.author.pubkey}.png`,
  );
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <img
        src={picture}
        alt={displayName}
        className={compact ? "h-9 w-9 rounded-full" : "h-12 w-12 rounded-full"}
      />
      <div className="min-w-0">
        ...
        {!compact && profile?.about && (
          <div className="mt-1 line-clamp-2 text-sm text-base-content/70">
            {profile.about}
          </div>
        )}
      </div>
    ...
    <div className="grid grid-cols-[repeat(auto-fill,minmax(10rem,1fr))] gap-3">
      {pack.emojis.map((emoji) => (
        ...
        >
          ...
          <div className="mt-3 truncate font-mono text-sm">
            :{emoji.shortcode}:
          </div>
          {emoji.address && (
            <div className="mt-1 text-xs text-base-content/60">
              {emoji.address.identifier}
            </div>
          )}
        </div>
      ))}
    ...
    <section className="flex-1 w-full h-full rounded-2xl bg-base-100 p-2 sm:p-5 overflow-y-auto overflow-x-hidden">
      <div className="flex flex-col gap-4 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 truncate font-mono text-sm text-base-content/60">
            {pack.identifier}
          </div>
          {pack.description && (
            <p className="mt-3 max-w-2xl text-sm text-base-content/80">
              {pack.description}
            </p>
          )}
          <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-2 text-sm text-base-content/70">
          <div className="rounded-full bg-base-200 px-3 py-1">
...
export default function EmojiPacksExample() {
      ...
      relay
        ? pool
            ...
            .pipe(
              ...
              mapEventsToTimeline(),
              castTimelineStream(EmojiPack, eventStore),
            )
        : undefined,
    [relay],
  );
  ...
  const selectedPack =
    filteredPacks?.find((pack) => pack.id === selectedId) ?? filteredPacks?.[0];
