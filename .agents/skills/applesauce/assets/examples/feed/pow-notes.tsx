...
function Note({ note }: { note: NostrEvent }) {
  const profile = useProfile(
    useMemo(
      () => ({
        pubkey: note.pubkey,
        relays: mergeRelaySets(getSeenRelays(note)),
      }),
      [note],
    ),
  );
  ...
  const distributionData = useMemo(() => {
    ...
    for (let difficulty = 0; difficulty <= maxDifficulty; difficulty++) {
      const count = notes.filter(
        (note) => getPoWDifficulty(note) === difficulty,
      ).length;
    ...
    <div className="card bg-base-100 shadow-md mb-4">
      <div className="card-body">
        ...
        <p className="text-sm text-base-content/60 mb-4">
          Number of notes with exactly X difficulty bits
        </p>
        ...
        <div className="h-40 flex items-end gap-1 overflow-x-auto">
          {distributionData.map(({ difficulty, count }) => (
            ...
            >
              <div className="text-xs text-center mb-1 text-base-content/60">
                {count}
              </div>
              <div
              ...
              />
              <div className="text-xs text-center mt-1 writing-mode-vertical-lr text-base-content/60">
                {difficulty}
              </div>
            </div>
          ))}
...
export default function PoWNotes() {
  ...
  const timelineLoader = useMemo(
    () => createTimelineLoader(pool, [relay], { kinds: [1] }, { eventStore }),
    [relay],
  );
  ...
  const filteredAndSortedEvents = useMemo(() => {
    ...
    const filtered = allEvents.filter(
      (event) => getPoWDifficulty(event) >= minDifficulty,
    );
    ...
    <div className="container mx-auto my-8 px-4 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-base-content/70 mb-4">
          View kind 1 notes filtered by NIP-13 proof of work difficulty. Higher
          difficulty means more computational work was done to create the note.
        </p>
      </div>
      ...
      {allEvents && (
        <PoWDistributionGraph notes={allEvents} minDifficulty={minDifficulty} />
      )}
        ...
        allEvents.length > 0 ? (
          <div className="card bg-base-100 shadow-md">
            <div className="card-body text-center">
              <p className="text-lg">
                No notes found with difficulty ≥ {minDifficulty} bits
              </p>
              <p className="text-base-content/60">
                Try lowering the minimum difficulty
              </p>
            </div>
          </div>
        ) : filteredAndSortedEvents.length === 0 ? (
