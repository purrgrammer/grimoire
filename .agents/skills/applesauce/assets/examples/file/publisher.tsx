...
async function uploadFileToBlossom(
  ...
  const results = await (isMedia ? multiServerMediaUpload : multiServerUpload)(
    servers,
    file,
    {
      onAuth: (_server, sha256, type) =>
        createUploadAuth((draft) => signer.signEvent(draft), sha256, { type }),
      onStart: (server: URL) => hooks?.onStart?.(server),
      onUpload: (server: URL) => hooks?.onUpload?.(server),
      onError: (server: URL, uploadError: unknown) =>
        hooks?.onError?.(server, uploadError),
    },
  );
  ...
  if (file.type.startsWith("image/")) {
    return (
      <img
        src={preview}
        alt={file.name}
        className="block max-h-80 w-full max-w-full rounded-2xl object-contain"
      />
    );
  }
      ...
      >
        <div className="max-w-xl space-y-4 px-4">
          ...
          <p className="text-sm text-base-content/70">
            Upload one file to your blossom servers, turn it into a kind `1063`
            event with `FileMetadataFactory`, then sign and publish it to your
            outbox relays.
          </p>
          <div className="inline-flex rounded-full border border-base-300 px-4 py-2 text-sm">
            Choose file
          </div>
        </div>
      </button>
  ...
  return (
    <ul className="space-y-2 text-xs font-mono">
      {results.map((result) => (
        <li
          key={result.relay}
          className="flex items-center gap-2 rounded-xl border border-base-300 px-3 py-2"
        >
    ...
    <ul className="space-y-2 text-xs font-mono">
      {statuses.map((status) => (
        ...
        >
          ...
          {status.url && (
            <span className="truncate text-base-content/50">{status.url}</span>
          )}
          {status.message && (
            <span className="truncate text-base-content/50">
              {status.message}
...
function Publisher({ user, signer }: { user: User; signer: ISigner }) {
  ...
  const publishRelays = useMemo(
    () =>
      mailboxes?.outboxes?.length
        ? relaySet(FALLBACK_RELAYS, mailboxes.outboxes)
        : FALLBACK_RELAYS,
    [mailboxes],
  );
  ...
  const [draftPreview, setDraftPreview] = useState<Awaited<
    ReturnType<typeof FileMetadataFactory.fromUpload>
  > | null>(null);
  ...
  const [results, setResults] = useState<
    { relay: string; ok: boolean; message?: string }[] | null
  >(null);
  ...
  const publishedCast = useMemo(
    () =>
      publishedEvent
        ? castEvent(publishedEvent, FileMetadata, eventStore)
        : null,
    [publishedEvent],
  );
  const authorProfile = use$(
    () => publishedCast?.author.profile$,
    [publishedCast?.author.pubkey],
  );
  ...
  const handleFile = async (file: File) => {
    ...
    setResults(null);
    setUploadStatuses(
      (blossomServers?.length ? blossomServers : DEFAULT_BLOSSOM_SERVERS).map(
        (server) => ({
          server: server.href,
          state: "pending",
        }),
    ...
    setUploading(true);
    try {
      const uploaded = await uploadFileToBlossom(
        file,
        signer,
        blossomServers,
            ...
            setUploadStatuses((current) =>
              current.map((status) =>
                status.server === server.href
                  ? { ...status, state: "uploading", message: undefined }
                  : status,
              ),
            ),
          onUpload: (server) =>
            setUploadStatuses((current) =>
              current.map((status) =>
                status.server === server.href
                  ? { ...status, state: "uploaded", message: undefined }
                  : status,
              ),
            ),
  ...
  const displayName = publishedCast
    ? getDisplayName(
        authorProfile,
        publishedCast.author.pubkey.slice(0, 8) + "...",
      )
    : undefined;
  const avatar = publishedCast
    ? getProfilePicture(
        authorProfile,
        `https://robohash.org/${publishedCast.author.pubkey}.png`,
      )
    : undefined;
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-4 p-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-base-content/70">
              Upload a file to your blossom servers, review the resulting kind
              `1063` metadata event, then sign and publish it to your NIP-65
              outbox relays.
            </p>
          </div>
      ...
      {error && (
        <section className="rounded-2xl border border-error/40 bg-base-100 p-4 text-sm text-error">
          {error}
        </section>
      )}
      ...
      {selectedFile && !uploading && (
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <section className="min-w-0 space-y-4 rounded-3xl border border-base-300 bg-base-100 p-5">
            ...
            <FilePreview
              file={selectedFile}
              remoteUrl={uploadResult?.metadata.url}
            />
            ...
            {uploadResult && (
              <div className="rounded-2xl border border-base-300 p-4 text-sm">
                ...
                <div className="mt-3 space-y-2 text-base-content/80">
                  ...
                  <div>
                    MIME type: {uploadResult.metadata.type || "Unknown"}
                  </div>
                  <div>
                    Stored on {uploadResult.publishedURLs.length} blossom
                    server(s)
                  </div>
                </div>
                <div className="mt-4">
              ...
              <div className="rounded-2xl border border-base-300 p-4">
                ...
                <div className="mt-4 flex items-start gap-3">
                  <img
                    src={avatar}
                    alt={displayName}
                    className="h-12 w-12 rounded-full border border-base-300"
                  />
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold">{displayName}</div>
                    <div className="text-xs text-base-content/60">
                      {publishedCast.createdAt.toLocaleString()}
                    </div>
                    <a
          ...
          <section className="min-w-0 space-y-4 rounded-3xl border border-base-300 bg-base-100 p-5">
            <div>
              ...
              <div className="mt-1 text-sm text-base-content/60">
                Edit the optional fields that will be added to the file metadata
                event.
              </div>
            </div>
            ...
            <MetadataField
              ...
              onChange={(thumbnail) =>
                setFields((current) => ({ ...current, thumbnail }))
              }
              placeholder="https://..."
            />
            <MetadataField
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <MetadataField
                ...
                onChange={(dimensions) =>
                  setFields((current) => ({ ...current, dimensions }))
                }
              ...
              <MetadataField
                ...
                onChange={(blurhash) =>
                  setFields((current) => ({ ...current, blurhash }))
                }
            ...
            <div className="rounded-2xl border border-base-300 p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {publishRelays.map((relay) => (
                  <div
                    key={relay}
                    className="rounded-full border border-base-300 px-2 py-1 text-xs font-mono"
                  >
                    {relay.replace(/^wss:\/\//, "")}
                  </div>
                ))}
              ...
              <div className="rounded-2xl border border-base-300 p-4">
                ...
                <ul className="mt-3 max-h-64 space-y-1 overflow-y-auto text-xs font-mono text-base-content/80">
                  {draftPreview.tags.map((tag, index) => (
                    <li key={`${tag[0]}-${index}`}>
                      [{tag.map((part) => JSON.stringify(part)).join(", ")}]
                    </li>
                  ))}
                </ul>
              </div>
