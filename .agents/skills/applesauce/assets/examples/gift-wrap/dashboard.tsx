...
function Username({ pubkey, relays }: { pubkey: string; relays?: string[] }) {
  ...
  const displayName = getDisplayName(
    profile,
    npubEncode(pubkey).slice(0, 5 + 4) + "…" + npubEncode(pubkey).slice(-4),
  );
...
export default function GiftWrapDashboard() {
  ...
  const notices = useObservableEagerMemo(
    () => pool.relay(relay).notices$,
    [relay],
  );
  ...
  const giftWrapEvents = use$(
    () => eventStore.timeline({ kinds: [kinds.GiftWrap] }),
    [],
  );
  ...
  const recipientStats = useMemo(() => {
    ...
    for (const event of giftWrapEvents) {
      ...
      if (receiver)
        recipientMap.set(receiver, (recipientMap.get(receiver) || 0) + 1);
    }
      ...
      <div className="card bg-base-100">
        <div className="card-body">
          ...
          <p className="text-sm text-base-content/70 mb-4">
            View all gift wrap events on a relay, grouped by recipient. The
            table is sorted by the number of gift wrap events sent to each
            recipient.
          </p>
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  {authenticated ? (
                    <div className="badge badge-success">
                      {pubkey && (
                        <span>
                          Authenticated as {npubEncode(pubkey).slice(0, 9)}...
                          {npubEncode(pubkey).slice(-4)}
                        </span>
                      )}
              ...
              {!needsAuth && (
                <div className="badge badge-ghost">No Auth Required</div>
              )}
            </div>
            {latestNotice && (
          ...
          {giftWrapEvents &&
            giftWrapEvents.length > 0 &&
            recipientStats.length === 0 && (
              <div className="alert alert-warning">
                <span>
                  Found {giftWrapEvents.length} gift wrap event(s), but none
                  have valid recipient tags.
                </span>
              </div>
            )}
        </div>
