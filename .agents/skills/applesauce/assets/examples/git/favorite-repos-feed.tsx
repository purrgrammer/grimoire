...
function RepositoryRow({ favorite }: { favorite: RepositoryFavorite }) {
  const repo = use$(
    () =>
      eventStore
        .replaceable(favorite.pointer)
        .pipe(castEventStream(GitRepository, eventStore)),
    [favorite.address],
  );
  const ownerProfile = use$(
    () => eventStore.profile(favorite.pointer.pubkey),
    [favorite.pointer.pubkey],
  );
  const ownerName = getDisplayName(
    ownerProfile,
    favorite.pointer.pubkey.slice(0, 8) + "...",
  );
  const ownerPicture = getProfilePicture(
    ownerProfile,
    `https://robohash.org/${favorite.pointer.pubkey}.png`,
  );
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <div className="text-sm text-base-content/60">
              Loading repository announcement...
            </div>
          </div>
    ...
    <article className="border border-base-300 bg-base-100 p-4">
      <div className="flex flex-col gap-4 md:flex-row md:items-start">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-bold leading-tight truncate">
                {repo.name ?? favorite.pointer.identifier}
              </h2>
          ...
          {repo.description && (
            <p className="text-sm text-base-content/80 mb-3 max-w-3xl">
              {repo.description}
            </p>
          )}
          ...
          <div className="flex flex-col gap-2 text-sm">
            {repo.cloneUrls[0] && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <code className="bg-base-200 px-2 py-1 text-xs break-all">
                  {repo.cloneUrls[0]}
                </code>
                <button
        ...
        <div className="md:w-56 md:border-l md:border-base-300 md:pl-4">
          ...
          <div className="text-xs text-base-content/60">
            ...
            {new Date(
              Math.max(
                ...favorite.favoritedBy.map((list) => list.event.created_at),
              ) * 1000,
            ).toLocaleString()}
          </div>
        </div>
...
export default function FavoriteRepositoriesFeed() {
    ...
    () =>
      eventStore.timeline({ kinds: [FAVORITE_GIT_REPOS_KIND] }).pipe(
        ...
        map((lists) => {
          ...
          for (const list of lists) {
            for (const pointer of list.repositoryPointers) {
              ...
              if (favorite) favorite.favoritedBy.push(list);
                ...
                favoritesByAddress.set(address, {
                  pointer,
                  address,
                  favoritedBy: [list],
                });
          ...
          return [...favoritesByAddress.values()].sort(
  ...
  const listsCount = new Set(
    favoriteRows.flatMap((favorite) =>
      favorite.favoritedBy.map((list) => list.event.id),
    ),
  ).size;
  const publicPointers = favoriteRows.reduce(
    (sum, favorite) => sum + favorite.favoritedBy.length,
    0,
  );
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8">
      <div className="mb-5 flex gap-4 justify-between items-center">
        <RelayPicker
          value={relay}
          onChange={setRelay}
          common={["wss://relay.ngit.dev/", "wss://gitnostr.com/"]}
        />
      ...
      {!favorites ? (
        <div className="border border-base-300 p-6 text-center">
          ...
          <p className="mt-3 text-base-content/70">
            Listening for public Git repository lists...
          </p>
        </div>
      ) : favoriteRows.length === 0 ? (
        <div className="border border-base-300 p-6 text-center">
          <h2 className="font-semibold">
            No public Git repository favorites found yet
          </h2>
          <p className="text-sm text-base-content/70 mt-2">
            Try another relay, or wait for kind {FAVORITE_GIT_REPOS_KIND} events
            to arrive.
          </p>
        </div>
      ) : (
