...
function GraspServerManagerView({ user }: { user: User }) {
  ...
  const publishSigned = useCallback(
    async (factory: GitGraspListFactory) => {
      ...
      try {
        ...
        console.error(err);
        setError(
          err instanceof Error ? err.message : "Failed to publish grasp list",
        );
      }
    },
    [signer, outboxes],
  ...
  const handleAdd = useCallback(
    async (url: string) => {
      ...
      try {
        if (graspList)
          await publishSigned(
            GitGraspListFactory.modify(graspList.event).addServer(url),
          );
        else await publishSigned(GitGraspListFactory.create().addServer(url));
      } catch (err) {
        console.error(err);
        setError(
          err instanceof Error ? err.message : "Failed to add grasp server",
        );
      }
    },
  ...
  const handleRemove = useCallback(
    async (url: string) => {
      ...
      try {
        await publishSigned(
          GitGraspListFactory.modify(graspList.event).removeServer(url),
        );
      } catch (err) {
        console.error(err);
        setError(
          err instanceof Error ? err.message : "Failed to remove grasp server",
        );
      }
    },
    [graspList, signer, outboxes, publishSigned],
    ...
    <div className="container mx-auto p-4 max-w-4xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
        <div>
          ...
          <p className="text-base-content/70">
            Manage NIP-34 grasp service relays (kind 10317). Lists are built
            with <code className="text-xs">GitGraspListFactory</code> and shown
            via the <code className="text-xs">GitGraspList</code> cast on{" "}
            <code className="text-xs">gitGraspList$</code>.
          </p>
        </div>
        ...
        <div className="mb-6 bg-base-100 rounded">
          ...
          <div className="text-sm flex flex-wrap gap-1">
            {outboxes.map((relay) => (
              <span
                key={relay}
                className="text-xs bg-base-200 rounded px-2 py-1 font-mono"
              >
      ...
      <div>
        <h2 className="text-lg font-semibold mb-4">
          Your Grasp Servers ({servers.length})
        </h2>

        {outboxes && outboxes.length === 0 ? (
          <div className="alert alert-warning mb-4">
            <span>
              No outbox relays yet. Publish a NIP-65 relay list so your grasp
