...
function graspServersFromRepoTags(event: NostrEvent): string[] {
  return relaySet(
    processTags(event.tags, (tag) => (tag[0] === "g" ? tag[1] : undefined)),
  );
}
...
function MaintainerChip({ user }: { user: User }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-0 max-w-[14rem] rounded border border-base-300 bg-base-200/40 px-2 py-1">
      ...
      <span className="truncate text-xs font-medium text-base-content/90">
        {displayName}
      </span>
    </div>
  );
...
function EndpointChip({
  ...
  const display = use$(() => {
    ...
    return combineLatest([r.icon$, r.information$]).pipe(
      map(([icon, info]) => ({
        icon,
        name:
          (info?.name && info.name.trim()) || endpointHostname(url) || relayUrl,
      })),
    );
  }, [pool, url]);
  ...
  return (
    ...
    >
      ...
      <span className="truncate font-medium text-base-content/90">
        {display?.name ?? endpointHostname(url)}
      </span>
    </span>
  );
}
...
function RepoCard({
  ...
  const repo = useMemo(
    () => castEvent(event, GitRepository, eventStore),
    [event],
  );
  ...
  const ownerPicture = getProfilePicture(
    profile,
    `https://robohash.org/${event.pubkey}.png`,
  );
  ...
  const upstreamWorkshopUrl = upstream
    ...
    : "";
  const upstreamNostrNaddrUri = upstream
    ? `nostr:${naddrEncode(upstream.pointer)}`
    : "";
                    ...
                    <div className="w-9 h-9 rounded-full border border-base-300">
                      <img
                        src={getProfilePicture(
                          upstreamProfile,
                          `https://robohash.org/${upstreamPubkey}.png`,
                        )}
                        alt=""
                      />
                    </div>
          ...
          <div className="text-xs text-base-content/60 mb-2 space-y-1">
            ...
            {repo.earliestUniqueCommit && (
              <div>
                ...
                <code className="bg-base-200 px-1">
                  {repo.earliestUniqueCommit}
                </code>
              </div>
            )}
...
export default function RepoSearchFeed() {
  ...
  const relayHints = useMemo(
    () =>
      [relay, "wss://relay.ngit.dev/", "wss://relay.damus.io/"].filter(Boolean),
    [relay],
  );
      ...
      {loading && repos === null ? (
        <div className="border border-base-300 p-6 text-center">
          ...
          <p className="mt-3 text-base-content/70">
            Loading repository announcements…
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {repos.map((ev) => (
            <RepoCard
              key={getReplaceableAddress(ev) ?? ev.id}
              pool={pool}
              event={ev}
