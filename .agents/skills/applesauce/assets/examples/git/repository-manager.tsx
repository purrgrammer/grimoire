...
function RepositoryEditor({
  ...
  const upstreamRepos = use$(
    () =>
      upstreamAuthor
        ? upstreamAuthor.timeline$(GIT_REPOSITORY_KIND, GitRepository)
        : of([] as GitRepository[]),
    [upstreamAuthor],
  );
    ...
    <form
      ...
      onSubmit={handleSubmit((values) =>
        onSave({
          ...
          maintainers: values.maintainers.map(
          ...
          upstreamIdentifier: upstreamEnabled
            ? values.upstreamIdentifier.trim()
            : "",
        ...
        <div className="flex flex-col gap-2">
          <div className="flex gap-2 items-start">
            ...
            <button
              ...
              disabled={
                !newMaintainer.trim() ||
                maintainers.some((item) => item.pubkey === newMaintainer.trim())
              }
            >
              Add
            </button>
          ...
          {maintainers.length === 0 ? (
            <div className="text-sm text-base-content/60 border border-base-300 p-3">
              No maintainers set.
            </div>
          ) : (
            <div className="flex flex-col gap-2">
      ...
      <div className="space-y-3">
        <label className="label cursor-pointer justify-start gap-2 py-0">
          ...
          <span className="label-text">
            Forked from another Nostr git repository
          </span>
        </label>
        <p className="text-xs text-base-content/60 -mt-1">
          Links this repo to the upstream pointer (NIP-34{" "}
          <code className="text-xs">u</code> tag). Uncheck to clear.
        </p>
          ...
          <>
            ...
            <div>
              ...
              {upstreamAuthor && upstreamIdentifierOptions.length === 0 ? (
                <p className="text-xs text-base-content/60 mt-1">
                  No repositories loaded for this author yet.
                </p>
              ) : null}
            </div>
            <div>
...
function RepositoryManagerView({ user }: { user: User }) {
  ...
  const repositories = use$(
    () => user.timeline$(GIT_REPOSITORY_KIND, GitRepository),
    [user.pubkey],
  );
  ...
  useEffect(() => {
    ...
    if (
      !selectedUid ||
      !orderedRepositories.some((repo) => repo.uid === selectedUid)
    ) {
      setSelectedUid(orderedRepositories[0].uid);
    }
  }, [orderedRepositories, selectedUid]);

  const selectedRepository = orderedRepositories.find(
    (repo) => repo.uid === selectedUid,
  );

  const handleSave = useCallback(
    async (input: RepoFormInput) => {
      if (!selectedRepository || !signer || !outboxes || outboxes.length === 0)
        return;
    ...
    <div className="container mx-auto max-w-5xl p-4">
      ...
      {!repositories ? (
        <div className="border border-base-300 p-4">
          Loading your repositories from outboxes...
        </div>
      ) : orderedRepositories.length === 0 ? (
        <div className="border border-base-300 p-4">
          No repositories found. Publish at least one kind {GIT_REPOSITORY_KIND}{" "}
          event first, then return here.
        </div>
      ) : (
