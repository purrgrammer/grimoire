...
function CommunityCard({ community }: { community: NostrEvent }) {
  ...
  const displayName =
    getDisplayName(profile) || community.pubkey.slice(0, 16) + "...";
  const picture = getProfilePicture(
    profile,
    `https://robohash.org/${community.pubkey}`,
  );
  const description =
    getTagValue(community, "description") ||
    profile?.about ||
    "No description available";
  const location = getTagValue(community, "location");
  const contentTypes = useMemo(
    () => getCommunityContentTypes(community),
    [community],
  );
  const relays = useMemo(
    () => community.tags.filter(isRTag).map((r) => r[1]),
    [community],
  );
  ...
  return (
    <div className="card bg-base-100 border border-base-300">
      <div className="card-body p-4">
        ...
        {location && (
          <div className="text-xs opacity-60 mb-2">📍 {location}</div>
        )}
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-col gap-1">
              {relays.map((relay, index) => (
                <div
                  key={index}
                  className="text-xs opacity-60 truncate font-mono"
                >
                  {relay}
                </div>
              ))}
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-base-content/60">
          No communities were found on this relay. Try selecting a different
          relay.
        </p>
      </div>
    );
...
export default function CommunikeysExample() {
  ...
  const communities = use$(
    () => eventStore.timeline({ kinds: [COMMUNITY_CREATION_KIND] }),
    [eventStore],
  );
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="mb-8">
        ...
        <p className="text-base-content/60 mb-6">
          Browse communities using the NIP-CC Communi-keys specification. Select
          a relay to discover communities hosted on it.
        </p>
      ...
      {!selectedRelay && (
        <div className="text-center py-12">
          ...
          <p className="text-base-content/60">
            Choose a relay from the dropdown above to start browsing
            communities.
          </p>
        </div>
      )}
