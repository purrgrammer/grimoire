...
function GroupCard({ group, relay }: { group: NostrEvent; relay: string }) {
  ...
  const name =
    groupPointer.name || getTagValue(group, "name") || "Unnamed Group";
    ...
    <div className="card bg-base-100 border border-base-300 hover:border-primary transition-colors">
      <div className="card-body p-4">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {isPrivate && (
                <div className="badge badge-soft badge-warning">Private</div>
              )}
              {isPublic && (
                <div className="badge badge-soft badge-success">Public</div>
              )}
              {isOpen && (
                <div className="badge badge-soft badge-info">Open</div>
              )}
              {isClosed && (
                <div className="badge badge-soft badge-error">Closed</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="break-all bg-base-200 p-2 rounded select-all">
            {groupPointerString}
          </div>
        </div>
      </div>
...
function GroupsGrid({
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-base-content">
          No groups were found on this relay. Try selecting a different relay or
          check if the relay supports NIP-29.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {groups.map((group) => (
        <GroupCard
          key={`${group.pubkey}-${getTagValue(group, "d") || "_"}`}
          group={group}
          relay={relay}
        />
      ))}
...
export default function GroupsExample() {
  ...
  const authRequiredForRead = useObservableEagerMemo(
    () =>
      selectedRelay
        ? pool.relay(selectedRelay).authRequiredForRead$
        : undefined,
    [selectedRelay],
  );
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="mb-8">
        ...
        <p className="text-base-content mb-6">
          Browse groups using the NIP-29 specification. Select a relay to
          discover groups hosted on it. Each group card displays the group
          pointer which can be selected and copied for use in other examples.
        </p>
              ...
              <div className="alert alert-warning">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs">
                    This relay requires authentication to read groups.
                  </div>
                </div>
                <button
      ...
      {!selectedRelay && (
        <div className="text-center py-12">
          ...
          <p className="text-base-content">
            Choose a NIP-29 relay from the dropdown above to start browsing
            groups.
          </p>
        </div>
      )}
