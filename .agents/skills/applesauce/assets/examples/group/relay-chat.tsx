...
function ChatMessageGroup({ messages }: { messages: NostrEvent[] }) {
  const profile = use$(
    () =>
      eventStore.profile({
        pubkey: messages[0].pubkey,
        relays: mergeRelaySets(getSeenRelays(messages[0])),
      }),
    [messages[0].pubkey],
  );
        ...
        <div className="w-10 rounded-full">
          <img
            ...
            src={getProfilePicture(
              profile,
              `https://robohash.org/${messages[0].pubkey}`,
            )}
          />
        </div>
      </div>
...
function ChatLog({ pointer }: { pointer: GroupPointer }) {
  const messages = use$(
    () =>
      pool
        .relay(pointer.relay)
        .subscription(
          { kinds: [9], "#h": [pointer.id], limit: 100 },
          { reconnect: true },
        )
        .pipe(
          // map to store
  ...
  const groups = groupMessageEvents(
...
function SendMessageForm({ pointer }: { pointer: GroupPointer }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const signed = await GroupMessageFactory.create(pointer, message).sign(
        signer,
      );
          ...
          <div className="alert alert-warning">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs">
                This relay requires authentication to read group messages.
              </div>
              {challenge && (
                <div className="text-xs mt-1">
                  Challenge received: {challenge.slice(0, 10)}...
                </div>
              )}
            </div>
            <button
        ...
        <div className="mb-4">
          <div className="alert alert-info">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs">
                This relay supports authentication but it's not required to read
                messages.
              </div>
            </div>
            <button
