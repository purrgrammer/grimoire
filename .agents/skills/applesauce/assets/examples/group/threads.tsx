...
function ThreadCard({
  ...
  const profile = use$(
    () =>
      eventStore.profile({
        pubkey: event.pubkey,
        relays: mergeRelaySets(getSeenRelays(event)),
      }),
    [event.pubkey],
  );
...
function ReplyForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const signed = await CommentFactory.create(event, content)
        .group(pointer)
        .sign(signer);
      // Publish the event
  ...
  return (
    <form
      ...
      className="flex gap-2 mt-4 flex-col max-w-xl mx-auto my-4"
    >
      <textarea
...
function ThreadView({
  ...
  const replies = use$(
    () => eventStore.model(CommentsModel, event),
    [event.id],
  );
...
function NewThreadForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const signed = await GroupThreadFactory.create(
        pointer,
        title,
        content,
      ).sign(signer);
  ...
  return (
    <div className="container mx-auto p-2 h-full">
      ...
      {pointer && !selectedThread && (
        <ThreadsList pointer={pointer} onSelect={setSelectedThread} />
      )}
      {pointer && selectedThread && (
        <>
          <button
