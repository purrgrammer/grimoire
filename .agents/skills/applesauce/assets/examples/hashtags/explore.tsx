...
function EventCard({
  ...
  const components = useMemo<ComponentMap>(
    () => ({
      ...
      hashtag: ({ node }) => (
        <button
          onClick={() => onHashtagClick(node.hashtag)}
  ...
  const hashtags = useMemo(
    () =>
      event.tags.filter((t) => t[0] === "t" && t[1]).map((t) => t[1] as string),
    [event],
  );
...
export default function HashtagExploreExample() {
  ...
  const relay = useMemo(
    () => (relayUrl ? pool.relay(relayUrl) : null),
    [relayUrl],
  );
  ...
  const events = useMemo(
    () =>
      (useLocalFiltering ? localEvents : relayEvents)?.filter((e) =>
        e.tags.some(isTTag),
      ),
    ...
    <div className="container mx-auto p-2 h-full">
      ...
      {relayUrl && !supportsNip91 && supportedNips !== null && (
        <div className="alert alert-warning mb-4">
          ...
          <span>
            This relay does not support NIP-91 (AND tag filters). Multiple
            hashtag filtering may not work as expected.
          </span>
        </div>
      )}
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        {useLocalFiltering && (
          <span className="text-sm text-base-content/70">
            Local filtering uses AND logic for multiple hashtags, even if relay
            doesn't support NIP-91
          </span>
        )}
      </div>
      ...
      <div className="mb-4">
        <label className="label">
          <span className="label-text">
            Filter by hashtags (space or comma separated)
          </span>
        </label>
        <input
          type="text"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {events && events.length > 0 ? (
            ...
            <div className="col-span-2 text-center text-base-content/70 py-8">
              No events with hashtags found. Try selecting a different relay or
              adjusting your hashtag filters.
            </div>
          )}
        </div>
      ) : (
        <div className="text-center text-base-content/70 py-8">
          Please select a relay to start exploring hashtags.
        </div>
      )}
    </div>
  );
