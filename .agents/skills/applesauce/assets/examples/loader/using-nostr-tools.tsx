...
createEventLoaderForStore(store, upstream, {
...
function NoteEvent({ note }: { note: Note }) {
  ...
  const replyingToNote = replyingToEvent
    ? castEvent(replyingToEvent, Note, note.store)
    : null;
...
export default function UsingNostrToolsExample() {
  ...
  const loader = useMemo(
    () =>
      createTimelineLoader(upstream, [relay], [{ kinds: [1] }], {
        eventStore: store,
        limit: 20,
      }),
    [relay],
