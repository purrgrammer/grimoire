...
function MessageGroup({
  ...
  return (
    <div className={`chat ${isOwn ? "chat-end" : "chat-start"}`}>
      ...
      <div
        className={`flex flex-col-reverse gap-2 overflow-hidden ${isOwn ? "items-end" : "items-start"}`}
      >
...
function ConversationList({
  ...
  const conversations = useMemo(() => {
    ...
    return Array.from(convMap.entries()).sort(
      (a, b) => b[1].lastMessage.created_at - a[1].lastMessage.created_at,
    );
  }, [messages]);
