...
const cacheRequest: CacheRequest = (filters) =>
  getEventsForFilters(cache, filters);
...
function ContactItem({
  ...
  const profile = use$(
    () => eventStore.model(ProfileModel, contactPubkey),
    [contactPubkey],
  );
  const displayName = getDisplayName(
    profile,
    contactPubkey.slice(0, 8) + "...",
  );
  const avatar = getProfilePicture(
    profile,
    `https://robohash.org/${contactPubkey}.png`,
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <div>{displayName}</div>
        <div className="text-xs font-semibold opacity-60 break-all">
          {npubEncode(contactPubkey)}
        </div>
      </div>
    </li>
  );
...
function Message({
  ...
  const content = use$(
    () => eventStore.model(EncryptedContentModel, message),
    [message.id],
  );
  ...
  return (
    <div>
      <span
        className={`font-bold ${sender === pubkey ? "text-primary" : "text-secondary"}`}
      >
        {sender.slice(0, 8)}
      </span>
      :{" "}
...
function DirectMessageView({
  ...
  const loader$ = useMemo(
    () =>
      createTimelineLoader(pool, [relay], filters, {
        eventStore,
        cache: cacheRequest,
      }),
  ...
  const messages = useObservableEagerMemo(
...
function HomeView({ pubkey, signer }: { pubkey: string; signer: ISigner }) {
  ...
  const timeline = useMemo(
    () => createTimelineLoader(pool, [relay], filters, { eventStore }),
    [relay],
  );
  useEffect(() => {
