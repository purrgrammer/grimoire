...
const expiration$ = new BehaviorSubject<string>(
  localStorage.getItem("note-expiration") ?? "3 days",
);
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        {expirationText && (
          <span
            className={clsx(
              "text-xs",
              expirationText === "Expired" ? "text-error" : "opacity-50",
            )}
          >
            • {expirationText}
          </span>
    ...
    <div className="border border-base-300 rounded-lg p-4">
      <div className="cursor-pointer" onClick={onSelect}>
        <div className="whitespace-pre-line">
          {selected ? note.content : truncatedContent}
        </div>
        <div className="flex items-center justify-between mt-2">
          ...
          <div className="flex items-center gap-2">
            {comments.length > 0 && (
              <span className="badge badge-sm">{comments.length} comments</span>
            )}
          </div>
        </div>
      </div>
...
function NotesView({ user }: { user: User }) {
      ...
      !dmRelays
        ...
        : pool
            .sync(
              dmRelays,
              eventStore,
              { kinds: [kinds.GiftWrap], "#p": [user.pubkey], since },
          ...
          map((messages) =>
            messages.filter(
              ...
              (msg) =>
                ...
                getConversationParticipants(msg).every(
                  (p) => p === user.pubkey,
                ),
            ),
          ),
  ...
  const rootNotes = useMemo(
    () =>
      messages
        .filter((note) => !getWrappedMessageParent(note))
      ...
      <div className="flex flex-col gap-3 p-4 bg-base-100 border-b border-base-300">
        ...
        <div className="flex justify-between">
          <div className="text-sm opacity-70">
            {rootNotes.length} notes • {synced} events synced •{" "}
            {messages.length} unlocked • {locked?.length ?? 0} locked •{" "}
