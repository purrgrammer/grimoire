...
const actions = new ActionRunner(
  eventStore,
  new ProxySigner(signer$.pipe(defined())),
  async (event) => {
    // Get the users outboxes
    const mailboxes = await firstValueFrom(
      eventStore.mailboxes(event.pubkey).pipe(defined(), simpleTimeout(5_000)),
    );
...
function MuteHashtagForm({
  onAdd,
}: {
  onAdd: (hashtag: string, hidden: boolean) => Promise<void>;
}) {
...
function MuteWordForm({
  onAdd,
}: {
  onAdd: (word: string, hidden: boolean) => Promise<void>;
}) {
  const [input, setInput] = useState("");
  const [hidden, setHidden] = useState(false);
...
function MutePubkeyForm({
  onAdd,
}: {
  onAdd: (pubkey: string, hidden: boolean) => Promise<void>;
}) {
...
function MuteThreadForm({
  onAdd,
}: {
  onAdd: (thread: EventPointer, hidden: boolean) => Promise<void>;
}) {
  const [input, setInput] = useState("");
  const [hidden, setHidden] = useState(false);
    ...
    <>
      ...
      {showError && (
        <label className="label">
          <span className="label-text-alt text-error">
            Only nevent is supported for muting threads
          </span>
        </label>
      )}
      ...
      {pointer && isEventPointer(pointer) ? (
        ...
        <div className="alert alert-error">
          <span>
            Only event threads (nevent) are supported for muting. Address
            threads (naddr) are not supported.
          </span>
        </div>
      ) : null}
...
function MutedPubkeyItem({
  ...
  const displayName =
    profile?.displayName ?? npubEncode(pubkey).slice(0, 8) + "...";
...
function MutedThreadItem({
  ...
  if (isEventPointer(pointer)) {
    ...
    const displayName =
      profile?.displayName ?? note.author.npub.slice(0, 8) + "...";
      ...
      <div className="card bg-base-100 shadow-sm">
        <div className="card-body">
          <div className="card-title flex gap-2 items-center">
            <div className="avatar">
              <div className="w-10 rounded-full">
                <img
                  src={
                    profile?.picture ??
                    `https://robohash.org/${note.author.pubkey}.png`
                  }
                  alt={displayName}
                />
              </div>
            ...
            <div>{displayName}</div>
            <div className="ms-auto text-sm opacity-60">
    ...
    const article = use$(
      () =>
        eventStore
          .replaceable(pointer)
          .pipe(castEventStream(Article, eventStore)),
        ...
        <div role="alert" className="alert alert-vertical sm:alert-horizontal">
          ...
          <div>
            <h3 className="font-bold">
              Loading article {getReplaceableAddressFromPointer(pointer)}...
            </h3>
          </div>
          {onRemove && (
            <button
    ...
    const displayName =
      profile?.displayName ?? article.author.npub.slice(0, 8) + "...";
            ...
            <div className="avatar">
              <div className="w-10 rounded-full">
                <img
                  src={
                    profile?.picture ??
                    `https://robohash.org/${article.author.pubkey}.png`
                  }
                  alt={displayName}
                />
              </div>
...
function MuteManager({ user }: { user: User }) {
  ...
  // Get public mutes
  const publicHashtags = useMemo(
    () => Array.from(mutes?.hashtags ?? []),
    [mutes?.hashtags],
  );
  const publicWords = useMemo(
    () => Array.from(mutes?.words ?? []),
    [mutes?.words],
  );
  const publicPubkeys = useMemo(
    () => Array.from(mutes?.pubkeys ?? []),
    [mutes?.pubkeys],
  );
  const publicThreads = useMemo(
    () => Array.from(mutes?.threads ?? []),
    [mutes?.threads],
  );

  // Get hidden mutes
  const hiddenHashtags = useMemo(
    () => Array.from(hidden?.hashtags ?? []),
    [hidden?.hashtags],
  );
  const hiddenWords = useMemo(
    () => Array.from(hidden?.words ?? []),
    [hidden?.words],
  );
  const hiddenPubkeys = useMemo(
    () => Array.from(hidden?.pubkeys ?? []),
    [hidden?.pubkeys],
  );
  const hiddenThreads = useMemo(
    () => Array.from(hidden?.threads ?? []),
    [hidden?.threads],
  );
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      alert(
        err instanceof Error ? err.message : "Failed to unlock hidden mutes",
      );
    } finally {
      setUnlocking(false);
    }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm opacity-70">
                  Unlock to view hidden mutes
                </p>
              </div>
              <button
                className="btn btn-primary"
                onClick={handleUnlock}
                disabled={unlocking || !signer}
              >
                {unlocking ? (
      ...
      <div className="space-y-6">
        ...
        <div className="card bg-base-100 shadow-md">
          <div className="card-body">
            ...
            <MuteHashtagForm
              onAdd={(hashtag, hidden) => handleMuteHashtag(hashtag, hidden)}
            />
            <div className="mt-4">
        ...
        <div className="card bg-base-100 shadow-md">
          <div className="card-body">
            <h3 className="text-xl font-bold mb-3">
              Words ({publicWords.length + (hidden?.words.size ?? 0)})
            </h3>
            <MuteWordForm
              onAdd={(word, hidden) => handleMuteWord(word, hidden)}
            />
            <div className="mt-4">
              <div className="tabs tabs-lift">
                <input
        ...
        <div className="card bg-base-100 shadow-md">
          <div className="card-body">
            <h3 className="text-xl font-bold mb-3">
              Users ({publicPubkeys.length + (hidden?.pubkeys.size ?? 0)})
            </h3>
            <MutePubkeyForm
              onAdd={(pubkey, hidden) => handleMutePubkey(pubkey, hidden)}
            />
            <div className="mt-4">
              <div className="tabs tabs-lift">
                <input
        ...
        <div className="card bg-base-100 shadow-md">
          <div className="card-body">
            <h3 className="text-xl font-bold mb-3">
              Threads ({publicThreads.length + (hidden?.threads.size ?? 0)})
            </h3>
            <MuteThreadForm
              onAdd={(thread, hidden) => handleMuteThread(thread, hidden)}
            />
            <div className="mt-4">
              <div className="tabs tabs-lift">
                <input
