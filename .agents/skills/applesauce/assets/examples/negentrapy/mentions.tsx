      ...
      {!mentions || mentions.length === 0 ? (
        <div className="text-center py-8 text-base-content/60">
          {active ? (
            ...
            <p>
              No mentions found. Set a pubkey and start a sync to load mentions.
            </p>
          )}
        </div>
      ) : (
...
export default function MentionsExample() {
  ...
  use$(() => {
    if (!pubkey || !mailboxes?.inboxes || mailboxes.inboxes.length === 0)
      return NEVER;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6 w-full">
      ...
      <div className="space-y-4">
        ...
        {pubkey && mailboxes && mailboxes.inboxes.length === 0 && (
          <div className="bg-error/10 border-l-4 border-error p-4">
            ...
            <div className="text-sm">
              <p className="mb-2">
                This user hasn't published a NIP-65 relay list or has no inbox
                relays configured.
              </p>
              <p className="text-xs text-base-content/60">
                Without inbox relays, we can't sync mentions for this user. They
        ...
        {mailboxes && mailboxes.inboxes.length > 0 && (
          <div className="bg-success/10 border-l-4 border-success p-4">
            <h3 className="font-bold text-success mb-2">
              Inbox Relays Detected - Syncing Mentions
            </h3>
            <div className="text-sm">
              <div className="mb-3">
                <p className="mb-2">
              ...
              <p className="text-xs text-base-content/60 mt-2">
                Using inbox relays to sync mentions where this user was tagged
      ...
      <SyncedMentions
        pubkey={pubkey}
        active={
          !!(pubkey && mailboxes?.inboxes && mailboxes.inboxes.length > 0)
        }
        since={since}
      />
    </div>
