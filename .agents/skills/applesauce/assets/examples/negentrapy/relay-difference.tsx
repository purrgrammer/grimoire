...
const syncIdsFromRelay = async (
  ...
  if (!supported?.includes(77))
    throw new Error("Relay does not support NIP-77 (Negentropy)");
...
function SyncResultsTable({
  ...
  const validResults = useMemo(() => {
    return results.filter((result) => {
      ...
      return (
        result.loading ||
        !result.error ||
        !result.error.includes("does not support NIP-77")
      );
    });
  }, [results]);
  ...
  const eventsWithDifferences = useMemo(() => {
    return allEventIds.filter((eventId) => {
      ...
      const relaysWithEvent = validResults.filter(
        (result) =>
          !result.loading && !result.error && result.eventIds.includes(eventId),
      ).length;

      // Count total valid relays that have completed syncing
      const completedRelays = validResults.filter(
  ...
  const profile = use$(
    () => (pubkey ? eventStore.profile({ pubkey }) : of(null)),
    [pubkey],
  );
  ...
  if (validResults.length === 0) {
    return (
      <div className="text-center py-8 text-base-content/60">
        <p>
          No relays with NIP-77 support found. Only relays supporting Negentropy
          (NIP-77) can be compared.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="badge badge-primary">
          {eventsWithDifferences.length} events with differences
        </div>
        {eventsWithDifferences.length < allEventIds.length && (
          <div className="badge badge-ghost">
            {allEventIds.length - eventsWithDifferences.length} events hidden
            (present on all relays)
          </div>
        )}
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-base-content/60">
                Comparing outbox relays for this user
              </p>
            </div>
          </div>
        </div>
        ...
        {validResults.map((result) => {
          const percentage =
            allEventIds.length > 0
              ? Math.round((result.eventIds.length / allEventIds.length) * 100)
              : 0;

          return (
            <div
              key={result.relay}
              className="stat bg-base-200 overflow-hidden p-2 rounded-md"
            >
              ...
              <div className="stat-value text-lg overflow-hidden">
                {result.loading ? (
                  ...
                  <span className="text-error text-sm truncate">
                    {result.error}
                  </span>
                ) : (
...
export default function RelayDifferenceExample() {
  ...
  const since = useMemo(
    () => unixNow() - timeFilter.hours * 60 * 60,
    [timeFilter],
  );
  ...
  useEffect(() => {
    ...
    const syncPromises = mailboxes.outboxes.map(async (relay, index) => {
      try {
        const eventIds = await syncIdsFromRelay(
          relay,
          filter,
          controller.signal,
        );
        setSyncResults((prev) =>
          prev.map((result, i) =>
            i === index ? { ...result, eventIds, loading: false } : result,
          ),
        );
      } catch (error) {
        setSyncResults((prev) =>
      ...
      <div className="space-y-4">
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <select
            ...
            onChange={(e) => {
              const selectedFilter = TIME_FILTERS.find(
                (f) => f.hours === parseInt(e.target.value),
              );
        ...
        {pubkey && mailboxes && mailboxes.outboxes.length === 0 && (
          <div className="bg-error/10 border-l-4 border-error p-4">
            ...
            <div className="text-sm">
              <p className="mb-2">
                This user hasn't published a NIP-65 relay list or has no outbox
                relays configured.
              </p>
              <p className="text-xs text-base-content/60">
                Without outbox relays, we can't compare relay differences. They
                need to publish their relay preferences using a NIP-65
                compatible client.
              </p>
            </div>
          </div>
        ...
        {mailboxes && mailboxes.outboxes.length > 0 && (
          <div className="bg-success/10 border-l-4 border-success p-4">
            <h3 className="font-bold text-success mb-2">
              Outbox Relays Detected - Syncing Event IDs
            </h3>
            <div className="text-sm">
              <div className="mb-3">
                <p className="mb-2">
              ...
              <p className="text-xs text-base-content/60 mt-2">
                Using Negentropy (NIP-77) to compare event IDs across your
                outbox relays without transferring the actual events.
              </p>
            </div>
          </div>
        )}
