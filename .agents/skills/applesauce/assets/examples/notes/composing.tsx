...
function createBlossomUploader(
  ...
  return async (attrs: FileAttributes): Promise<UploadTask> => {
    ...
    try {
      const results = await (
        isMedia ? multiServerMediaUpload : multiServerUpload
      )(servers, file, {
        onAuth: (_server, sha256, type) =>
          createUploadAuth((draft) => signer.signEvent(draft), sha256, {
            type,
          }),
      });
  ...
  const editor = useEditor(
    {
      extensions: [
        ...
        NostrExtension.configure({
          fileUpload: {
            ...
            upload: (attrs) =>
              createBlossomUploader(
                signer,
                blossomServersRef.current,
                mediaOptimizationRef.current,
              )(attrs),
          },
          link: { autolink: true },
        }),
      ...
      editorProps: {
        attributes: {
          class:
            "min-h-[220px] px-3 py-2 outline-none focus:outline-none max-w-none text-base leading-relaxed",
        },
      },
    },
    [],
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <p className="text-base-content/70 text-sm">
        Use the editor to write a kind 1 note. Mentions, links, and hashtags are
        turned into the right Nostr content and tags when you build the event.
        Drag and drop or attach images and videos — they are uploaded to your
        blossom server automatically.
      </p>
      <label className="flex flex-wrap items-center gap-2 cursor-pointer text-sm text-base-content/80 w-fit">
        ...
        <span>
          ...
          <span className="text-base-content/50">
            (BUD-05 /media when the server supports it)
          </span>
        </span>
      </label>
        ...
        {editor && (
          <div className="flex items-center gap-2 px-3 py-2 border-t border-base-300">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              />
              {!blossomServers?.length && (
                <span className="text-xs text-base-content/50">(default)</span>
              )}
            </div>
          </div>
        )}
...
function PublishBody({
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      setLastResults(
        responses.map((r) => ({ relay: r.from, ok: r.ok, message: r.message })),
      );
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <p className="text-base-content/70 text-sm">
        This is the unsigned kind 1 draft from{" "}
        <code className="text-xs bg-base-200 px-1 rounded">NoteFactory</code>.
        Signing uses your extension; events are sent to your NIP-65 outbox
      ...
      <section className="border border-base-300 rounded-lg p-4 space-y-2">
        <h2 className="font-semibold text-sm text-base-content/60">
      ...
      <section className="border border-base-300 rounded-lg p-4 space-y-2">
        ...
        {!mailboxes?.outboxes?.length && (
          <p className="text-xs text-base-content/50">
            No outbox list loaded yet — using default relays.
          </p>
        )}
      </section>
          ...
          <ul className="space-y-1 text-xs font-mono">
            {lastResults.map((r) => (
              <li key={r.relay} className="flex gap-2 items-center">
                <span className={r.ok ? "text-success" : "text-error"}>
                  {r.ok ? "ok" : "fail"}
                </span>
