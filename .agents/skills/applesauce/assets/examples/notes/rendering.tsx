    ...
    void (async () => {
      ...
      if (src.startsWith("blossom:")) {
        ...
        const extraFallbacks =
          hintPubkey && getServersForPubkey
            ? ((await getServersForPubkey(hintPubkey)) ?? [])
            : [];

        const urls = await Actions.getBlobUrls(src, {
          getServers: getServersForPubkey
            ? async (pubkey: string) => getServersForPubkey(pubkey)
            : undefined,
...
async function getBlossomServersForPubkey(pubkey?: string) {
  ...
  return firstValueFrom(
    eventStore
      ...
      .pipe(defined(), take(1), timeout(5000)),
  );
}
...
function BlossomRenderer({ node }: { node: BlossomURI }) {
  ...
  if (isImageURL(probe))
    inner = (
      <img src={node.raw} className="max-h-64 rounded" alt="Blossom image" />
    );
  ...
  else if (isVideoURL(probe))
    inner = <video src={node.raw} className="max-h-64 rounded" controls />;
  else if (isAudioURL(probe))
    inner = <audio src={node.raw} className="max-h-64 rounded" controls />;
...
function NoteCard({ note }: { note: Note }) {
  ...
  use$(
    () => eventStore.blossomServers({ pubkey: author.pubkey }),
    [author.pubkey],
  );
  ...
  return (
    <NoteAuthorPubkeyContext.Provider value={author.pubkey}>
      <div
        ref={cardRef}
        data-pubkey={author.pubkey}
        className="p-4 border border-base-300 rounded-lg overflow-hidden"
      >
        <div className="flex items-center gap-3 mb-3">
          <div className="avatar">
            <div className="w-10 rounded-full">
              <img
                src={
                  profile?.picture ??
                  `https://robohash.org/${author.pubkey}.png`
                }
                alt=""
              />
            </div>
          ...
          <div className="min-w-0 flex-1">
            <div className="font-semibold truncate">
              {profile?.displayName || author.npub.slice(0, 8)}
            </div>
