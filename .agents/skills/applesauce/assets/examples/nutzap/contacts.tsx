...
const actions = new ActionRunner(
  ...
  (event, relays) => pool.publish(relays ?? [], event),
);
...
function Username({ user }: { user: User }) {
  ...
  return (
    <>{profile?.displayName ?? npubEncode(user.pubkey).slice(0, 12) + "..."}</>
  );
}
...
function ZapModal({
  ...
  useEffect(() => {
    if (nutzapInfo && nutzapInfo.mints.length > 0) {
      if (
        !selectedMint ||
        !nutzapInfo.mints.some((m) => m.mint === selectedMint)
      ) {
  ...
  const handleZap = async () => {
    ...
    try {
      ...
      try {
        ...
        // Create nutzap event
        await actions.run(NutzapProfile, contact.pubkey, tokens, {
          comment,
          couch,
        });
        ...
        {status === "paid" && (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm opacity-70">
              Your nutzap has been successfully sent
            </p>
            <div className="modal-action">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs opacity-70 truncate">
              {npubEncode(contact.pubkey).slice(0, 16)}...
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <button
              className="btn btn-primary btn-sm"
              onClick={() => onZap(contact)}
              disabled={canZap === false}
            >
              ⚡ Zap
            </button>
          </div>
      ...
      {contacts !== undefined && contacts.length === 0 && (
        <div className="text-center py-8">
          <div className="alert alert-info max-w-md mx-auto">
            <span>
              You don't have any contacts yet. Follow some people to see them
              here!
            </span>
          </div>
        </div>
      ...
      {contacts && contacts.length > 0 && (
        <>
          <div className="mb-4">
            ...
            <p className="text-sm opacity-70">
              Click the zap button to send sats to your contacts
            </p>
          </div>
