...
function ParsedUriDisplay({
  parsedUri,
}: {
  parsedUri?: WalletConnectURI | null;
}) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Object.entries(METHOD_DESCRIPTIONS).map(([method, description]) => {
          ...
          return (
            <div
              key={method}
              className="card card-compact card-border bg-base-200"
            >
              <div className="card-body">
                <div className="flex items-center justify-between">
                  <code className="text-sm font-mono">{method}</code>
                  <span
                    className={`badge ${isSupported ? "badge-success" : "badge-neutral"}`}
                  >
...
function EncryptionSection({ walletInfo }: { walletInfo: WalletSupport }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Object.entries(ENCRYPTION_DESCRIPTIONS).map(
          ([method, description]) => {
            const isSupported = supportsEncryption(
              walletInfo,
              method as WalletConnectEncryptionMethod,
            );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(NOTIFICATION_DESCRIPTIONS).map(
            ([notification, description]) => {
              const isSupported =
                walletInfo.notifications?.includes(notification as any) ??
                false;
              return (
                <div
                  key={notification}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="stat">
            <div className="stat-value text-primary">
              {getSupportedMethods(walletInfo).length}
            </div>
            <div className="stat-desc">Payment Methods</div>
          </div>
          <div className="stat">
            <div className="stat-value text-secondary">
              {walletInfo.encryption.length}
            </div>
            <div className="stat-desc">Encryption Methods</div>
          </div>
          <div className="stat">
            <div className="stat-value text-accent">
              {getSupportedNotifications(walletInfo).length}
            </div>
    ...
    <div className="container mx-auto my-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="mb-4 opacity-70">
          Enter a nostr+walletconnect URI to parse it and fetch wallet
          information.
        </p>
