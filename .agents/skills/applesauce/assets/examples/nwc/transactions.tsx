...
function TransactionItem({ transaction }: { transaction: Transaction }) {
  ...
  const settledDate = transaction.settled_at
    ? new Date(transaction.settled_at * 1000)
    : null;
  const expiresDate = transaction.expires_at
    ...
    <div className="w-full p-4 border border-base-300 rounded-lg bg-base-100">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          />
          <div
            className={`badge ${isIncoming ? "badge-success" : "badge-info"}`}
          >
        ...
        <div className="text-right">
          <div
            className={`font-bold text-lg ${isIncoming ? "text-success" : "text-info"}`}
          >
            {isIncoming ? "+" : "-"}
          ...
          {feesSats > 0 && (
            <div className="text-xs opacity-60">
              Fee: {feesSats.toLocaleString()} sats
            </div>
          )}
        </div>
      </div>
    ...
    <>
      ...
      <div className="space-y-2">
        {transactions.map((transaction, index) => (
          <TransactionItem
            key={`${transaction.payment_hash}-${index}`}
  ...
  const fetchTransactions = useCallback(
    async (page: number = 0, append: boolean = false) => {
      ...
      if (support && !supportsMethod(support, "list_transactions")) {
        setMethodError(
          "The connected wallet does not support the 'list_transactions' method.",
        );
        return;
      }
    ...
    <div className="container mx-auto my-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="mb-4 opacity-70">
          Enter a nostr+walletconnect URI to connect to a wallet and view its
          transaction history using the list_transactions method.
        </p>
        ...
        {transactions.length === 0 && !loading && support && !methodError && (
          <div className="text-center py-12">
            ...
            <p className="opacity-40">
              Connect to a wallet and fetch transactions to see them here.
            </p>
          </div>
        )}
      </div>
