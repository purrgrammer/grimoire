            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <span className="ml-2 badge badge-outline">
                  {walletInfo.block_height?.toLocaleString()}
                </span>
              </div>
              <div>
                <span className="font-medium">Block Hash:</span>
                <code className="ml-2 text-xs break-all">
                  {walletInfo.block_hash}
                </code>
              </div>
            </div>
          </div>
...
export default function WalletInfoExample() {
  ...
  const fetchWalletInfo = async () => {
    ...
    if (support && !supportsMethod(support, "get_info")) {
      setMethodError(
        "The connected wallet does not support the 'get_info' method.",
      );
      return;
    }
    ...
    <div className="container mx-auto my-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="mb-4 opacity-70">
          Enter a nostr+walletconnect URI to connect to a wallet and fetch its
          information using the get_info method.
        </p>
                  ...
                  {support && (
                    <div>
                      ...
                      <code className="ml-2 text-sm">
                        {getPreferredEncryption(support)}
                      </code>
                    </div>
                  )}
                </div>
