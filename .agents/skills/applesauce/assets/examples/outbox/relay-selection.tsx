...
function UserPreviewModal({ selection }: { selection?: ProfilePointer[] }) {
  ...
  const selected = selection?.find(
    (user) => user.pubkey === previewUser?.pubkey,
  );

  const displayName = profile
    ? getDisplayName(profile, previewUser?.pubkey.slice(0, 8) + "...")
    : "";
  ...
  const manual =
    mailboxes &&
    selected?.relays?.filter(
      (relay) =>
        !mailboxes?.inboxes.includes(relay) &&
        !mailboxes?.outboxes.includes(relay),
    );
    ...
    try {
      await firstValueFrom(
        eventStore.replaceable({
          kind: 10002,
          pubkey: previewUser.pubkey,
          relays: previewUser.relays,
        }),
      );
    } catch (error) {
...
function RelayRow({
  ...
  const relayDisplayName =
    info?.name || relay.replace("wss://", "").replace("ws://", "");
...
function UnhealthyRelay({ relay }: { relay: string }) {
  ...
  const isInBackoff = state?.backoffUntil
    ? Date.now() < state.backoffUntil
    : false;
  const backoffRemaining = state?.backoffUntil
    ? Math.max(0, state.backoffUntil - Date.now())
    : 0;
  ...
  return (
    <div className="flex items-center gap-2  p-2 hover:bg-base-100 border-l-2 border-l-error">
      ...
      <div className="flex items-center gap-2">
        <div
          className={`badge badge-sm ${getStateBadge(state?.state || "unknown")}`}
        >
          {state?.state || "unknown"}
        </div>
        {isInBackoff && backoffRemaining > 0 && (
          <div className="text-xs text-base-content/60">
            Backoff: {formatBackoffTime(backoffRemaining)}
          </div>
        )}
      </div>
      {state?.failureCount && (
      ...
      {state?.state === "dead" && (
        <button
          className="btn btn-xs btn-outline btn-error ms-auto"
          onClick={handleRevive}
          title="Revive this relay"
        >
          Revive
        </button>
      )}
...
function UsersByRelayCount({
  ...
  const missingRelaysUsers = useMemo(() => {
    ...
    return selection.filter((selectedUser) => {
      ...
      const hasNoRelaysInOriginal =
        !originalUser?.relays || originalUser.relays.length === 0;
      const hasNoRelaysInSelected =
        !selectedUser.relays || selectedUser.relays.length === 0;
  ...
  return (
    <div className="mt-8">
      ...
      <p className="text-base-content/60 mb-6">
        All users grouped by how many relays have been selected for them. Click
        rows to expand and see users.
      </p>
                    ...
                    >
                      ...
                      <td>
                        <div className="badge badge-warning">
                          {missingRelaysUsers.length}
                        </div>
                      </td>
                      <td>
                        <span className="text-sm text-base-content/80">
                          Users without any relays (no NIP-65 relay list
                          published)
                        </span>
                      </td>
                    </tr>
                ...
                {sortedRelayCounts
                  ...
                  .map((relayCount) => {
                    ...
                    const getDescription = (count: number): string => {
                      if (count === 1)
                        return "High risk - single point of failure";
                      if (count <= 3)
                        return "Limited redundancy - may have connectivity issues";
                      if (count <= 5)
                        return "Good coverage with reasonable redundancy";
      ...
      <div className="mt-6 p-4 bg-base-200 rounded-lg">
        <div className="text-sm text-base-content/70">
          <strong>{selection.length}</strong> total users distributed across
          relay count groups
        </div>
            ...
            <div className="flex flex-wrap gap-4">
              {missingRelaysUsers.length > 0 && (
                <span className="text-warning">
                  <strong>{missingRelaysUsers.length}</strong> users missing
                  relays (no NIP-65 published)
                </span>
              )}
              {orphanedUsers.length > 0 && (
                <span className="text-error">
                  <strong>{orphanedUsers.length}</strong> users orphaned (lost
                  relays after selection)
                </span>
              )}
            </div>
          </div>
        ...
        {sortedRelayCounts.length > 0 && (
          <div className="text-xs text-base-content/60 mt-1">
            Active relay range:{" "}
            {Math.min(...sortedRelayCounts.filter((c) => c > 0))} -{" "}
            {Math.max(...sortedRelayCounts)} relays per user
          </div>
        )}
      </div>
    </div>
...
export default function RelaySelectionExample() {
  ...
  const outboxMap = useMemo(
    () => selection && groupPubkeysByRelay(selection),
    [selection],
  );

  const sortedRelays = outboxMap
    ? Object.entries(outboxMap).sort(([, a], [, b]) => b.length - a.length)
    : [];
  ...
  return (
    <div className="min-h-screen p-4">
      ...
      <RelayStatusModal
        pool={pool}
        isOpen={showRelayStatus}
        onClose={() => setShowRelayStatus(false)}
      />
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col w-full">
            <label className="label pb-1">
              <span className="label-text">
                Max relays per user: {maxRelaysPerUser}
              </span>
            </label>
            <input
              type="range"
              ...
              <th colSpan={3} className="text-left">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    onClick={() => setShowRelayStatus(true)}
                  >
                    <span className="text-xs">Status:</span>
                    <div className="badge badge-sm badge-primary">
                      {
                        Object.values(poolStatuses || {}).filter((s) => s.ready)
                          .length
                      }
