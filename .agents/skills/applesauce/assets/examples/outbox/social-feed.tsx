...
function Note({ note, selection }: { note: NostrEvent; selection?: string[] }) {
  ...
  const mailboxes = use$(
    () => eventStore.mailboxes(note.pubkey),
    [note.pubkey],
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <a
              className="font-semibold text-sm link"
              onClick={() => preview$.next({ pubkey: note.pubkey })}
            >
              {displayName}
            </a>
            ...
            <span className="text-xs text-base-content/60 ms-auto">
              From {selection?.length ?? 0} of {mailboxes?.outboxes.length ?? 0}{" "}
              outboxes
            </span>
          </div>
...
function UserAvatar({ user }: { user: ProfilePointer }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-xs max-w-16 overflow-hidden truncate">
        {displayName}
      </div>
    </div>
  );
        ...
        <div>
          ...
          >
            ...
            <div className="text-xs text-base-content/60 ml-6">
              Users without any relays (no NIP-65 published)
            </div>
          </button>
      ...
      {orphanedUsers.length > 0 && (
        <div>
          ...
          >
            ...
            <div className="text-xs text-base-content/60 ml-6">
              Users who lost all relays after selection
            </div>
          </button>
    ...
    <div className="flex flex-col gap-2">
      {relays.map((relay) => (
        <div key={relay} className="flex gap-2 items-center justify-between">
          ...
          <button
            className="btn btn-square btn-warning btn-soft btn-xs text-lg"
      ...
      <form className="flex gap-2" onSubmit={addRelay}>
        ...
        />
        <button
          className="btn btn-square btn-primary btn-soft btn-sm text-lg"
          type="submit"
          disabled={!input}
        >
          +
...
export default function SocialFeedExample() {
  ...
  const outboxMap = use$(
    () => selection$.pipe(map(createOutboxMap)),
    [selection$],
  );
  ...
  const feedEvents = use$(
    () =>
      selection
        ? eventStore.timeline({
            kinds: [1],
            authors: selection?.map((s) => s.pubkey),
          })
        : of([]),
    [selection],
  );
          ...
          {pubkey ? (
            <div className="min-h-screen">
              ...
              <div className="sticky top-0 bg-base-50/95 backdrop-blur-sm border-b border-base-300 p-2 mb-4 z-10">
                ...
                <p className="text-base-content/60 text-sm">
                  Connected to {sortedRelays.length} relays •{" "}
                  {selection?.length || 0} contacts
                </p>
              </div>
                  ...
                  <div className="divide-y divide-base-300">
                    {feedEvents.slice(0, 100).map((event) => (
                      <Note
                        key={event.id}
                        note={event}
                        selection={byPubkey?.[event.pubkey]}
                      />
                    ))}
                  </div>
                ) : (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-base-content/60 text-lg">
                Enter your npub in the sidebar to get started
              </p>
            </div>
          )}
        </div>
      ...
      <div className="w-80 bg-base-100 border-r border-base-300 flex flex-col">
          ...
          <div className="space-y-1 max-h-96 overflow-y-auto">
            {sortedRelays.length === 0 ? (
              <div className="text-center py-4 text-base-content/60">
                {pubkey
                  ? "Loading relays..."
                  : "Enter your npub to view relays"}
              </div>
            ) : (
        ...
        <div className="p-4">
          ...
          <p className="text-base-content/60 text-sm mb-4">
            Used when no relays are available for a user
          </p>
          <RelayManager
            relays={fallbacks}
            onChange={(relays) => fallbacks$.next(relays)}
          />
        </div>
