...
function ResponseItem({ response, options }: ResponseItemProps) {
  ...
  const selectedOptions = options.filter((option) =>
    responseOptions.includes(option.id),
  );
  ...
  const displayName =
    getDisplayName(profile) || npubEncode(response.pubkey).slice(0, 12) + "...";
  ...
  const handleSubmitVote = async () => {
    ...
    try {
      ...
      const signedResponse = await (
        comment.trim() ? response.comment(comment.trim()) : response
      ).sign(signer);

      // Publish to the selected relay
      await pool.publish(
        mergeRelaySets(selectedRelay, getPollRelays(poll)),
        signedResponse,
      );
        ...
        <div className="flex flex-col gap-2">
          {options.map((option) => (
            ...
            >
              ...
              />
              <span
                className="label-text font-medium truncate"
                title={option.label}
              >
                {option.label}
              </span>
...
function PollDetails({ poll, onBack, selectedRelay }: PollDetailsProps) {
  ...
  const handleVoteSubmitted = (newResponse: NostrEvent) => {
    setResponses((prev) =>
      [...prev, newResponse].sort((a, b) => a.created_at - b.created_at),
    );
  };
          ...
          <div className="card-body">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-base-content/70 mb-4">
              ...
              <span>
                ...
                {voteStats.totalVotes === 1 ? "response" : "responses"}
              </span>
              {endsAt && (
                <span
                  className={`badge ${isExpired ? "badge-error" : "badge-warning"}`}
                >
                  {isExpired ? "Expired" : "Ends"}{" "}
                  {new Date(endsAt * 1000).toLocaleDateString()}
                </span>
              )}
            </div>
              ...
              {options.map((option) => {
                ...
                const percentage =
                  voteStats.totalVotes > 0
                    ? (votes / voteStats.totalVotes) * 100
                    : 0;
                ...
                return (
                  ...
                  >
                    <span
                      className="font-medium truncate flex-1 mr-3"
                      title={option.label}
                    >
                      {option.label}
                    </span>
        ...
        <div className="flex justify-between items-center mt-4 text-xs text-base-content/60">
          ...
          {endsAt && !isExpired && (
            <span className="badge badge-warning badge-sm">
              Ends {new Date(endsAt * 1000).toLocaleDateString()}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="text-center py-12">
        ...
        <div className="text-sm text-base-content/40">
          {selectedRelay
            ...
            : "Select a relay to start browsing polls"}
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto my-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-base-content/70 mb-6">
          Browse and vote on NIP-88 polls from Nostr relays. Click on a poll to
          view details and cast your vote.
        </p>
