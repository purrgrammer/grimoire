...
export default function RelayDiscoveryAttributes() {
  ...
  use$(
    () =>
      relay
        ? relay.subscription(relayFilter).pipe(mapEventsToStore(eventStore))
        : undefined,
    [relay, relayFilter],
  );
  ...
  const chartOptions = useMemo(() => {
    ...
    const maxFontSize = Math.min(
      baseFontSize * 3,
      containerDimensions.height * 0.15,
    );
  ...
  const handleAttributeClick = (attribute: string) => {
    if (!attributeInput.includes(attribute)) {
      setAttributeInput((prev) =>
        prev.trim() ? `${prev.trim()} ${attribute}` : attribute,
      );
    }
  };
    ...
    <div className="container mx-auto p-2 h-full">
      <div className="flex gap-2 justify-between items-center mb-4">
        ...
        <RelayPicker
          ...
          common={["wss://relay.nostr.watch/", ...COMMON_RELAYS]}
        />
      </div>

      <Nip91Warning
        relayUrl={relayUrl}
        supportsNip91={supportsNip91}
...
function Nip91Warning({
  ...
  return (
    <div className="alert alert-warning mb-4">
      ...
      <span>
        This relay does not support NIP-91 (AND tag filters). Multiple attribute
...
function AttributeFilterInput({
  ...
  setAttributeInput,
  attributeFilters,
}: AttributeFilterInputProps) {
  return (
    <div className="mb-4">
      <label className="label">
        <span className="label-text">
          Filter by attributes (space or comma separated)
        </span>
      </label>
      <input
        type="text"
