...
function RelayListItem({
  relay,
  monitor,
}: {
  relay: RelayWithUsers;
  monitor: RelayMonitor | undefined;
}) {
  // Get relay discovery status from monitor
  const discovery = use$(
    () => (monitor ? monitor.relayStatus(relay.url) : undefined),
    [monitor?.uid, relay.url],
  );
    ...
    <div className="space-y-4 h-full overflow-hidden flex flex-col">
      <div className="p-4 flex flex-col gap-2">
        ...
        <p className="text-sm text-base-content/70">
          Select a monitor and a user to view their contacts' outbox relays
          aggregated by popularity. Each relay shows which users are using it
          and its latest status from the monitor.
        </p>
        ...
        {selectedPubkey && contacts && contacts.length > 0 && (
          <div className="text-sm text-base-content/70">
            ...
            {relaysByPopularity.length > 0 &&
              ` with ${relaysByPopularity.length} unique outbox relays`}
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-center h-full text-base-content/50">
          <div className="text-center">
            <p className="text-lg mb-2">
              Select a monitor and user to view their contacts' outbox relays
            </p>
