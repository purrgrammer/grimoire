  ...
  return (
    <div className="max-w-sm">
      <h4 className="font-bold text-lg mb-2">
        {relay.type === "inbox" ? "Inbox" : "Outbox"} Relay
      </h4>
      ...
      <div className="text-xs text-gray-600 mb-1">
        <strong>Location:</strong> {relay.position[0].toFixed(4)},{" "}
        {relay.position[1].toFixed(4)}
      </div>
    </div>
  );
}
...
export default function OutboxMap() {
    ...
    () =>
      pool
        ...
        .pipe(
          ...
          mapEventsToTimeline(),
          castTimelineStream(RelayMonitor),
        ),
    [relay],
  );
  ...
  const monitor = useMemo(
    () =>
      selectedMonitor
        ? monitors?.find((m) => m.author.pubkey === selectedMonitor)
        : undefined,
  ...
  const mailboxes = use$(
    () =>
      selectedPubkey
        ? eventStore.mailboxes({ pubkey: selectedPubkey })
        : undefined,
    [selectedPubkey],
  );
    ...
    <div className="space-y-2 h-full overflow-hidden flex flex-col">
      <div className="p-4 flex flex-col gap-2">
        ...
        <p className="text-sm text-base-content/70">
          Select a monitor, then a user to view their inbox and outbox relays
          (NIP-65) displayed on a map. Relays with geohash data from relay
          discovery events will appear as markers. Inbox relays have green
          borders, outbox relays have blue borders.
        </p>
        ...
        {!monitor && (
          <div className="text-sm text-base-content/70">
            Select a monitor to view relay discovery data
          </div>
        )}

        {selectedPubkey && !mailboxes && (
          <div className="text-sm text-base-content/70">
            Loading mailboxes...
          </div>
        )}
        ...
        {selectedPubkey && mailboxes && !hasRelays && (
          <div className="alert alert-warning">
            <span>
              This user has no inbox or outbox relays configured (NIP-65).
            </span>
          </div>
        )}
        ...
        {selectedPubkey && mailboxes && hasRelays && (
          <div className="flex gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>
                Inboxes: {inboxRelays.length} ({inboxRelaysWithLocation.length}{" "}
                with location)
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span>
                Outboxes: {outboxRelays.length} (
                {outboxRelaysWithLocation.length} with location)
              </span>
            </div>
          </div>
        )}
        ...
        {selectedPubkey &&
          ...
          mailboxes &&
          hasRelays &&
          relaysWithLocation.length === 0 && (
            <div className="alert alert-info">
              <span>
                No location data found for this user's relays. The selected
                monitor may not have geohash data for these relays.
              </span>
            </div>
          )}
      </div>
          ...
          <div className="flex items-center justify-center h-full text-base-content/50">
            <div className="text-center">
              <p className="text-lg mb-2">
                Select a monitor and user to view their mailbox relays
              </p>
