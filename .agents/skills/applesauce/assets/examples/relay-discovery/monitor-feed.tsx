...
function RelayDiscoveryItem({ discovery }: { discovery: RelayDiscovery }) {
  ...
  const faviconUrl = use$(
    () => (relayUrl ? pool.relay(relayUrl).icon$ : undefined),
    [relayUrl],
  );
    ...
    <div className="flex items-center gap-3 p-3 border-b border-base-300 hover:bg-base-200 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-4 text-xs text-base-content/60 mt-1">
          {discovery.rttOpen !== undefined && (
            <span>Open: {discovery.rttOpen}ms</span>
          )}
          {discovery.rttRead !== undefined && (
            <span>Read: {discovery.rttRead}ms</span>
          )}
          {discovery.rttWrite !== undefined && (
            <span>Write: {discovery.rttWrite}ms</span>
          )}
          {discovery.networkType && (
            <span>Network: {discovery.networkType}</span>
          )}
        </div>
      </div>
...
function RelayDiscoveryFeed({
  ...
  const discoveries =
    use$(() => {
      ...
      return pool
        .subscription(
          monitor.author.outboxes$.pipe(
            map((outboxes) => outboxes || [fallbackRelay]),
          ),
          {
        ...
        .pipe(
          ...
          mapEventsToTimeline(),
          castTimelineStream(RelayDiscovery),
        );
    }, [monitor.uid]) ?? [];
      ...
      {discoveries.length === 0 && (
        <div className="flex items-center justify-center h-full text-base-content/50">
          <div className="text-center">
            ...
            <p className="text-sm">
              The monitor may not have published any events recently.
            </p>
          </div>
        </div>
      )}
...
export default function MonitorFeed() {
  ...
  const monitor = useMemo(
    () =>
      selected
        ? monitors?.find((m) => m.author.pubkey === selected)
        : undefined,
    [monitors, selected],
  );
    ...
    <div className="space-y-4 h-full overflow-hidden flex flex-col">
      <div className="p-4 flex flex-col gap-4">
        ...
        <p className="text-sm text-base-content/70">
          Select a relay (fallback) and a monitor to see a live feed of all
          relay discovery events that monitor is producing.
        </p>
          ...
          <div className="flex items-center justify-center h-full text-base-content/50">
            <div className="text-center">
              <p className="text-lg mb-2">
                Select a monitor to view relay discovery events
              </p>
              <p className="text-sm">
                Use the pubkey picker above to get started
              </p>
            </div>
          </div>
        </div>
