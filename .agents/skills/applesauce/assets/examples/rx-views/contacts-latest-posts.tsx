    ...
    monitor$.pipe(
      ...
      switchMap((monitor) => {
        ...
        return source.pipe(
          ...
          filter(
            (map) =>
              Array.from(map.values()).filter((rtt) => rtt !== Infinity)
                .length >= 3,
          ),
          // Sort relays by RTT
          map((map) =>
...
function contactsFastestOutboxesView(
  ...
  User,
  { last: NostrEvent | undefined; relays: string[] }
>> {
  ...
  return contacts$.pipe(
    ...
    combineLatestByValue((contact) => {
      ...
      const last = eventStore
        .timeline({ kinds: [kinds.ShortTextNote], authors: [contact.pubkey] })
        .pipe(
      ...
      return combineLatest({
        last,
        relays: onlineOutboxes$.pipe(startWith([])),
      });
    }),
  );
}
...
function ContactLatestRow({
  ...
  const picture =
    profile?.picture ?? `https://robohash.org/${contact.pubkey}.png`;
  ...
  return (
    <div className="flex gap-3 p-3 border border-base-300 rounded-lg">
      ...
      <div className="min-w-0 flex-1">
        ...
        {relays.length > 0 && (
          <p
            className="text-xs text-base-content/40 mt-0.5 font-mono truncate"
            title={relays.join(", ")}
          >
            {relays.join(", ")}
          </p>
        ...
        {note ? (
          <div className="mt-2 p-2 rounded border border-base-300 bg-base-200/60">
            <p className="text-base text-base-content whitespace-pre-wrap wrap-break-word">
              {note.content}
            </p>
          </div>
        ) : (
...
export default function ContactsLatestPosts() {
  ...
  const [selectedMonitorPubkey, setSelectedMonitorPubkey] = useState(
  ...
  const monitor = useMemo(
    () =>
      monitors && selectedMonitorPubkey
        ? monitors.find((m) => m.author.pubkey === selectedMonitorPubkey)
        : undefined,
    [monitors, selectedMonitorPubkey],
  );
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto">
      <div>
        ...
        <p className="text-sm text-base-content/60">
          Load a user’s contacts, collect their NIP-65 outbox relays, pick the 3
          fastest by RTT from a single monitor, then query those relays for kind
          1 and show each contact’s latest note.
        </p>
      </div>
      ...
      {!selectedPubkey && (
        <p className="text-sm text-base-content/40">
          Enter a pubkey to load their contacts and latest posts.
        </p>
      )}

      {selectedPubkey && !monitor && (
        <p className="text-sm text-base-content/40">
          Select a relay monitor to rank outboxes by RTT.
        </p>
      )}
      ...
      {user && (
        <>
          ...
          {contactToLatestNote !== undefined &&
            entriesSortedByRecent.length === 0 && (
