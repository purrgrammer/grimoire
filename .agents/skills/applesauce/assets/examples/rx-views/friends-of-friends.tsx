...
export default function FriendsOfFriends() {
  ...
  const contactsMap = use$(
    () =>
      FriendsOfFriendsView(user$.pipe(filter((u): u is User => u !== null))),
    [],
  );
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto">
      <div>
        ...
        <p className="text-sm text-base-content/60">
          Browse a user's contact network. Click any contact to navigate to
          their connections.
        </p>
      </div>
      ...
      {!currentUser && (
        <p className="text-sm text-base-content/40">
          Enter a pubkey above to explore their network.
        </p>
      )}
          ...
          <div>
            ...
            {contacts !== null && contacts.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {contacts.map(
                  ([contact, { name, contacts: theirContacts }]) => (
                    <ContactCard
                      key={contact.pubkey}
                      contact={contact}
                      name={name}
