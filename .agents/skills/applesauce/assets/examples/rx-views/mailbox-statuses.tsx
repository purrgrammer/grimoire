...
function RttBadge({ ms }: { ms: number | undefined }) {
  ...
  const color =
    ms < 200 ? "text-success" : ms < 600 ? "text-warning" : "text-error";
...
function StatusBadge({ discovery }: { discovery: RelayStatus }) {
  ...
  if (discovery === null)
    return <span className="badge badge-ghost badge-sm">Not monitored</span>;
  if (discovery instanceof Error)
    return <span className="badge badge-error badge-sm">Error</span>;
  if (discovery.rttOpen === undefined)
...
function MonitorStatusRow({
  ...
  return (
    <tr className="bg-base-200/40">
      ...
      <td className="text-xs text-base-content/60">
        {disc?.supportedNIPs
          ? disc.supportedNIPs.slice(0, 5).join(", ") +
            (disc.supportedNIPs.length > 5 ? "…" : "")
          : "—"}
      </td>
    </tr>
  );
...
function RelayRow({
  ...
  url: string;
  statuses: Map<RelayMonitor, RelayStatus>;
}) {
      ...
      <tr>
        <td colSpan={7}>
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-mono text-xs truncate font-semibold">
              {url}
            </span>
          </div>
        </td>
...
function StatusTable({
  ...
  label: string;
  statusMap: RelayStatusMap | undefined;
}) {
...
export default function MailboxStatuses() {
  ...
  const [selectedMonitorPubkeys, setSelectedMonitorPubkeys] = useState<
    string[]
  ...
  const selectedMonitors = useMemo(
    () =>
      monitors?.filter((m) =>
        selectedMonitorPubkeys.includes(m.author.pubkey),
      ) ?? [],
    [monitors, selectedMonitorPubkeys],
  );

  const user = useMemo(
    () => (selectedPubkey ? castUser(selectedPubkey, eventStore) : undefined),
    [selectedPubkey],
  );

  const inboxStatuses = use$(
    () =>
      user && selectedMonitors.length > 0
        ? inboxStatusesView(user, selectedMonitors)
        : undefined,
  ...
  const outboxStatuses = use$(
    () =>
      user && selectedMonitors.length > 0
        ? outboxStatusesView(user, selectedMonitors)
        : undefined,
    [user?.pubkey, selectedMonitors.map((m) => m.uid).join(",")],
  );
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto">
      <div>
        ...
        <p className="text-sm text-base-content/60">
          Pick a user and relay monitors to check the health of their NIP-65
          inbox and outbox relays.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {monitors && monitors.length > 0 && (
          <MonitorPicker
            monitors={monitors}
            value={selectedMonitorPubkeys}
            onChange={setSelectedMonitorPubkeys}
          />
        )}
      ...
      {!selectedPubkey && (
        <p className="text-sm text-base-content/40">
          Enter a pubkey above to check their relay health.
        </p>
      )}

      {selectedPubkey && selectedMonitors.length === 0 && (
        <div className="alert alert-warning text-sm">
          No monitor selected — relay status data requires a monitor.
        </div>
      )}
