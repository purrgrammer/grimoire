...
function MetadataDistributionView(
  user: User | Observable<User>,
): Observable<Map<Relay, NostrEvent[] | Error>> {
...
function KindBadge({
  ...
  if (event) {
    ...
    return (
      ...
      >
        <span className="text-success text-xs font-medium leading-none">
          {label}
        </span>
...
function RelayRow({
  ...
  const kindByNumber = useMemo(() => {
    if (!events || events instanceof Error)
      return new Map<number, NostrEvent>();
  ...
  const presentCount =
    isError || isLoading
      ? 0
      : visibleLabels.filter((l) => kindByNumber.has(METADATA_KINDS[l])).length;
  ...
  return (
    <div className="border border-base-300 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-base-200/50 border-b border-base-300">
        ...
        <span className="font-mono text-xs font-semibold flex-1 truncate">
          {relay.url}
        </span>
        {isLoading && (
...
export default function MetadataDistribution() {
  ...
  const distribution = use$(
    () => (user ? MetadataDistributionView(user) : undefined),
    [user?.pubkey],
  );
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto">
      <div>
        ...
        <p className="text-sm text-base-content/60">
          Checks which metadata events (profile, contacts, relay lists, etc.)
          are present on each of a user's outbox relays.
        </p>
      </div>
      ...
      {!selectedPubkey && (
        <p className="text-sm text-base-content/40">
          Enter a pubkey above to inspect their relay metadata coverage.
        </p>
      )}
      ...
      {user && (
        <>
          ...
          {relayEntries !== null && relayEntries.length === 0 && (
            <p className="text-sm text-base-content/40">
              No outbox relays found for this user.
            </p>
          )}
