...
function ResultCard({
  ...
  const author = useMemo(
    () => castUser(event.pubkey, eventStore),
    [event.pubkey],
  );
...
export default function InboxMentionsSearch() {
  ...
  const supportMap = use$(() => {
    if (!inboxes || inboxes.length === 0)
      return of({} as Record<string, number[] | null | undefined>);
    ...
    <div className="container mx-auto max-w-4xl p-4 space-y-4">
      <div>
        ...
        <p className="text-sm opacity-70">
          Pick a user, then search one of their NIP-65 inbox relays for
          kind&nbsp;1 events that tag them via{" "}
          <code className="font-mono">#p</code>. Requires the relay to support
          NIP-50.
        </p>
      </div>
