    ...
    <div className="form-control w-full">
      ...
      {!extensionAvailable && (
        <div className="alert alert-warning mt-2">
          <span>
            Nostr extension not detected. Install a browser extension like nos2x
            or Alby.
          </span>
        </div>
      )}
...
const NostrConnectURISection = ({
  onStartProvider,
}: {
  onStartProvider: (uri: string) => void;
}) => {
    ...
    try {
      ...
      const metadata = {
        ...
        icons: url.searchParams.get("icons")
          ? url.searchParams.get("icons")!.split(",")
          : [],
        permissions: url.searchParams.get("permissions")
          ? url.searchParams.get("permissions")!.split(",")
          : [],
      };
            ...
            <div className="bg-base-100 p-3 rounded-lg">
              ...
              {parsedUri.metadata.permissions.length > 0 && (
                <p>
                  ...
                  {parsedUri.metadata.permissions.join(", ")}
                </p>
              )}
            </div>
            <button
    ...
    <div className="card bg-base-200 shadow-md">
      <div className="card-body">
        ...
        <p className="text-sm opacity-70 mb-4">
          Start the provider and wait for a client to connect. The bunker:// URI
          will be displayed as a QR code.
        </p>
...
export default function BunkerProvider() {
  ...
  const [selectedSigner, setSelectedSigner] = useState<
    "extension" | "generated"
  >("extension");
  const [generatedKey, setGeneratedKey] = useState<PrivateKeySigner | null>(
    null,
  );
  ...
  const handleGenerateKey = useCallback(async () => {
    ...
    addLog("info", "Generated new key", {
      pubkey: pubkey,
      nsec: bytesToHex(signer.key),
    });
  }, [addLog]);
  ...
  const createSigner = useCallback(async () => {
    try {
      ...
      addLog("error", "Failed to create signer", {
    ...
    async (uri: string) => {
      try {
        ...
        const newProvider = new NostrConnectProvider({
          ...
          onLogout: (client) => {
            addLog("disconnect", "Client logged out, session ended", {
              client,
            });
          },
          onConnect: (client, permissions) => {
            addLog("request", "Connect request received", {
              client,
              permissions,
            });
          ...
          onNip04Encrypt: (pubkey, plaintext, _client) => {
            addLog("request", "NIP-04 encrypt request received", {
              targetPubkey: pubkey,
              plaintext,
            });
          ...
          onNip04Decrypt: (pubkey, _ciphertext, _client) => {
            addLog("request", "NIP-04 decrypt request received", {
              targetPubkey: pubkey,
            });
            return true; // Always allow decryption
          },
          onNip44Encrypt: (pubkey, plaintext, _client) => {
            addLog("request", "NIP-44 encrypt request received", {
              targetPubkey: pubkey,
              plaintext,
            });
            return true; // Always allow encryption
          ...
          onNip44Decrypt: (pubkey, _ciphertext, _client) => {
            addLog("request", "NIP-44 decrypt request received", {
              targetPubkey: pubkey,
            });
            return true; // Always allow decryption
          },
  ...
  const handleStartAndWait = useCallback(async () => {
    try {
      ...
      addLog("info", "Provider started successfully, waiting for client", {
        bunkerUri: uri,
      });
    } catch (error) {
      addLog("error", "Failed to start provider", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }
  }, [selectedRelay, selectedSigner, generatedKey, addLog, createSigner]);
  ...
  const handleStopProvider = useCallback(async () => {
    if (provider) {
      try {
        ...
        addLog("error", "Failed to stop provider", {
          error: error instanceof Error ? error.message : "Unknown error",
        });
      }
    }
  ...
  return (
    <div className="container mx-auto my-8 px-4 py-8 max-w-6xl">
      <h1 className="text-3xl font-bold mb-8 text-center">
        Nostr Connect Provider Example
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <BunkerURISection
            onStartProvider={handleStartAndWait}
            bunkerUri={bunkerUri}
            showQR={showBunkerURI}
          />
