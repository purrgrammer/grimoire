  ...
  const sub = graphLoader({
    ...
    .subscribe({
      ...
      complete: () => {
        ...
        persistGraph().catch((err) =>
          console.error("Failed to persist social graph", err),
        );
      },
    });
...
function UserAvatar({ user }: { user: User }) {
  ...
  return (
    <div
      ...
      title={profile?.displayName ?? user.npub.slice(0, 12) + "..."}
    >
      <div
...
function DistanceRow({
  ...
  const pubkeys = useMemo(
    () => Array.from(graph.getUsersByFollowDistance(distance)).sort(),
    [graph, distance, update],
  );
  const preview = pubkeys.slice(0, 24);
  const users = useMemo(
    () => preview.map((pubkey) => castUser(pubkey, eventStore)),
    [preview.join("|")],
  );
  ...
  const loaded = useMemo(
    () =>
      pubkeys.reduce(
        (acc, pubkey) =>
          acc + (eventStore.hasReplaceable(kinds.Contacts, pubkey) ? 1 : 0),
        0,
      ) || 0,
    [pubkeys],
  );
    ...
    <div className="container mx-auto my-6 p-4 flex flex-col gap-4">
      <div>
        ...
        <p className="text-base-content/60 text-sm">
          ...
          rooted at a pubkey. The graph is saved to local storage so it can be
          reloaded later.
        </p>
      </div>
        ...
        <>
          ...
          <div>
            ...
            <PubkeyPicker
              ...
              placeholder="Enter an npub or nostr identifier..."
            />
          </div>
          ...
          <div className="flex items-center gap-3 p-3 border border-base-300 rounded-lg">
            <div
              className={`badge ${isRunning ? "badge-success" : "badge-ghost"} ${isRunning ? "animate-pulse" : ""}`}
            >
            ...
            <span className="text-sm">
              Loaded <span className="font-bold">{syncState?.loaded ?? 0}</span>{" "}
              follow events
            </span>
            {loadTime != null && (
              <span
                className="text-sm text-base-content/60"
                title="Time to load graph from local storage"
              >
                Loaded from storage in{" "}
                <span className="font-mono">{loadTime.toFixed(0)}ms</span>
              </span>
            )}
            {graph && (
              <span className="text-sm text-base-content/60 ml-auto">
                Graph size:{" "}
                <span className="font-mono">{graph.size().users}</span> users
              </span>
            )}
          </div>
