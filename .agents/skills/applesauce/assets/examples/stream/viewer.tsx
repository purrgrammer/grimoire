        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="avatar">
            <div className="w-8 rounded-full">
              <img
                src={
                  host?.picture || `https://robohash.org/${stream.host.pubkey}`
                }
          ...
          <span className="text-sm font-medium">
            {host?.displayName || stream.host.pubkey}
          </span>
    ...
    return (
      <div className="text-center py-12">
        <p className="text-base-content/60">
          No live streams found on this relay
        </p>
      </div>
    );
  }
      ...
      <div className="chat-image avatar">
        <div className="w-10 rounded-full">
          <img
            ...
            src={
              profile?.picture ||
              `https://robohash.org/${message.author.pubkey}`
            }
          />
        </div>
      </div>
...
function RelayName({ pool, url }: { pool: RelayPool; url: string }) {
  const name = use$(
    () =>
      pool.relay(url).information$.pipe(map((i) => i?.name?.trim() || null)),
    [url],
  );
...
function RelayFavicon({ pool, url }: { pool: RelayPool; url: string }) {
  ...
  return (
    <div className="avatar">
      <div
        className={`w-6 rounded-full border-2 ${status?.connected ? "border-success" : "border-error"}`}
      >
...
function StreamChatDebugModal({ stream }: { stream: Stream }) {
  ...
  const allRelays = useMemo(
    () => relaySet(inboxes, stream.relays, platform),
    [inboxes, platform, stream.relays],
  );
  const inboxSet = useMemo(() => new Set(relaySet(inboxes)), [inboxes]);
  const streamRelaySet = useMemo(
    () => new Set(relaySet(stream.relays)),
    [stream.relays],
  );
  const platformRelaySet = useMemo(
    () => new Set(relaySet(platform)),
    [platform],
  );
  const rawJson = useMemo(
    () => JSON.stringify(stream.event, null, 2),
    [stream.event],
  );

  const openModal = () => {
    (
      document.getElementById(STREAM_DEBUG_MODAL_ID) as HTMLDialogElement | null
    )?.showModal();
  };
      ...
      >
        {allRelays.length > 0 ? (
          <span
            ...
            title="Relays used for chat subscription and publish"
          >
            {allRelays.length}
          </span>
        ) : null}
      ...
      <dialog id={STREAM_DEBUG_MODAL_ID} className="modal">
        <div className="modal-box max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <p className="text-sm text-base-content/70 py-1">
            Chat relays and raw replaceable event JSON.
          </p>

          <div className="divider my-2">Chat relays</div>
          <p className="text-sm text-base-content/70">
            Host inboxes and stream <code className="text-xs">relays</code> tags
            merged for chat. Badges show each relay&apos;s role.
          </p>
          <div className="py-2 overflow-y-auto max-h-48">
            {allRelays.length === 0 ? (
              <div role="alert" className="alert">
                <span>
                  No host inboxes or stream relays configured for this chat.
                </span>
              </div>
            ) : (
              <ul className="list">
                        ...
                        <div className="flex flex-wrap gap-1 shrink-0 ms-auto">
                          {isInbox ? (
                            <span
                              ...
                              title="From host NIP-65 read relays"
                            >
                              inbox
                            </span>
                          ) : null}
                          {isStreamRelay ? (
                            <span
                              ...
                              title="From stream event relays tag"
                            >
                              stream
                            </span>
                          ) : null}
          ...
          <p className="text-sm text-base-content/70">
            Kind {stream.event.kind} replaceable —{" "}
            <code className="text-xs">JSON.stringify</code> of the event.
          </p>
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col-reverse gap-1 overflow-x-hidden">
        {chat && chat.length > 0 ? (
          chat.map((message) => (
            <ChatMessage key={message.id} message={message} relays={relays} />
          ))
        ) : (
...
function ChatMessageForm({
  ...
  const send = handleSubmit(async (values) => {
    ...
    const event = await StreamChatMessageFactory.create(
      stream.event,
      values.content,
    ).sign(signer);
    ...
    <div className="p-4 border-b border-base-300">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-base-content/70">
            by {host?.displayName || stream.host.npub}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className={`badge ${statusColor}`}>{status}</div>
          {viewers !== undefined && (
            <div className="badge badge-outline">{viewers} viewers</div>
          )}
        </div>
      </div>
            ...
            {streaming && status === "live" ? (
              ...
              <div className="text-center text-white">
                ...
                <p className="text-white/70">
                  No streaming URL found for this stream
                </p>
              </div>
            )}
          </div>
...
export default function StreamCastExample() {
  ...
  const streams = use$(
    () =>
      eventStore
        .timeline({ kinds: [kinds.LiveEvent] })
