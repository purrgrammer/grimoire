...
function ReactionItem({ reaction }: { reaction: Reaction }) {
  ...
  const picture =
    profile?.picture || `https://robohash.org/${reaction.author.pubkey}.png`;
...
function ZapItem({ zap }: { zap: Zap }) {
  ...
  const picture =
    profile?.picture || `https://robohash.org/${zap.sender.pubkey}.png`;
...
function TorrentItem({ torrent }: { torrent: Torrent }) {
  ...
  const picture =
    profile?.picture || `https://robohash.org/${torrent.author.pubkey}.png`;
      ...
      <div className="card-body">
        ...
        <div className="mb-3">
          <h4 className="font-bold text-lg mb-2">
            {torrent.title || "Untitled Torrent"}
          </h4>
        ...
        {zaps && zaps.length > 0 && (
          <div className="mt-2 pt-2 border-t border-base-300/50">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-lg">⚡</span>
              <span className="font-semibold">
                {zaps.length} zap{zaps.length !== 1 ? "s" : ""} • {totalZapSats}{" "}
                sats total
              </span>
            </div>
            <div className="flex flex-wrap gap-2">
                    ...
                    <ul className="list-disc list-inside mt-1 ml-4">
                      ...
                      {torrent.files.length > 10 && (
                        <li className="text-xs">
                          ... and {torrent.files.length - 10} more
                        </li>
                      )}
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside mt-1 ml-4">
                      ...
                      {torrent.trackers.length > 5 && (
                        <li className="text-xs">
                          ... and {torrent.trackers.length - 5} more
                        </li>
                      )}
                    </ul>
                  </div>
...
export default function TorrentFeed() {
  ...
  const torrents = use$(
    () =>
      eventStore
        .timeline({ kinds: [TORRENT_KIND] })
        .pipe(castTimelineStream(Torrent, eventStore)),
    [],
  );
  ...
  if (selectedTorrent) {
    return (
      <TorrentView
        ...
        onBack={() => selectedTorrent$.next(null)}
      />
    );
  }
        ...
        {torrents === undefined ? (
          ...
          <div className="text-center py-12">
            <p className="text-base-content/60">
              No torrents found. Try a different relay.
            </p>
          </div>
        ) : (
