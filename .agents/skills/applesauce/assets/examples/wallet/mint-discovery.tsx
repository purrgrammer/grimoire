  ...
  return (
    <div className="avatar">
      <div className={`${size} rounded-full`}>
        <img
          src={picture || `https://robohash.org/${user.pubkey}.png`}
          alt={user.pubkey}
        />
      </div>
    </div>
...
function MintInfoCard({
  ...
  const author = useMemo(
    () => castUser(mintInfo.event.pubkey, eventStore),
    [mintInfo.event.pubkey],
  );
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h2 className="card-title">
              {mintInfo.metadata?.name ||
                mintInfo.metadata?.display_name ||
                mintInfo.url}
            </h2>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {mintInfo.network && (
                <span className="badge badge-outline">
                  Network: {mintInfo.network}
                </span>
              )}
              {mintInfo.nuts && mintInfo.nuts.length > 0 && (
                <span className="badge badge-outline">
                  NUTs: {mintInfo.nuts.join(", ")}
                </span>
              )}
            </div>
          </div>
...
function RecommendationCard({
  ...
  const author = useMemo(
    () => castUser(recommendation.event.pubkey, eventStore),
    [recommendation.event.pubkey],
  );
...
function MintDetailsView({
  ...
  use$(() => {
    ...
    return pool
      .subscription(Array.from(relays), {
              ...
              <div className="space-y-2">
                ...
                {mintInfo.network && (
                  <div>
                    ...
                    <span>{mintInfo.network}</span>
                  </div>
                )}
                {mintInfo.nuts && mintInfo.nuts.length > 0 && (
                  <div>
                    <span className="font-semibold">Supported NUTs:</span>{" "}
                    <span>{mintInfo.nuts.join(", ")}</span>
                  </div>
                )}
                {mintInfo.metadata &&
                  Object.keys(mintInfo.metadata).length > 0 && (
                    <div>
...
export default function MintDiscovery() {
  ...
  const mints = use$(
    () =>
      eventStore
        .timeline({ kinds: [CASHU_MINT_INFO_KIND] })
        .pipe(castTimelineStream(MintInfo, eventStore)),
    [],
  );
    ...
    <div className="container mx-auto my-8 px-4">
      <div className="mb-4">
        ...
        <p className="text-base-content/70 mb-4">
          Browse Cashu mint information events (kind:38172) published on Nostr
          relays.
        </p>
        ...
        {mints && mints.length > 0 ? (
          mints.map((mintInfo) => (
            <MintInfoCard
              key={mintInfo.id}
              mintInfo={mintInfo}
              onClick={() => setSelectedMint(mintInfo)}
            />
          ))
        ) : (
