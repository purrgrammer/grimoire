...
export default function ZapLiveGraph() {
    ...
    (node: ZapNode) => {
      ...
      const sub = user.profile$.subscribe((profile) => {
        ...
        const picture = getProfilePicture(
          profile,
          `https://robohash.org/${node.pubkey}.png?size=128x128`,
        );
  ...
  const paintNode = useCallback(
    (node: ZapNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      if (
        node.imageEl &&
        node.imageEl.complete &&
        node.imageEl.naturalWidth > 0
      ) {
  ...
  const paintLink = useCallback(
    (link: ZapLink, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      ctx.lineWidth =
        (0.5 + Math.min(4, Math.log2(link.amount + 1) / 3)) / globalScale;
      ctx.beginPath();
      ctx.moveTo(sx, sy);
      ctx.lineTo(tx, ty);
      ctx.stroke();
  ...
  const drawStars = useCallback(
    (ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      for (const star of starsRef.current) {
        const twinkle =
          0.4 + 0.6 * (0.5 + 0.5 * Math.sin(t * star.speed + star.phase));
        ctx.fillStyle = `rgba(255, 255, 255, ${twinkle * 0.85})`;
        ctx.beginPath();
        ctx.arc(
          star.x,
          star.y,
          star.r / Math.max(0.5, globalScale),
          0,
          Math.PI * 2,
        );
        ctx.fill();
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at center, #0b1030 0%, #06091c 45%, #02030a 100%)",
      }}
    >
      <ForceGraph2D
        ref={fgRef}
        ...
        linkDirectionalParticles={(l) =>
          Math.min(6, Math.ceil(Math.log2((l as ZapLink).amount + 1)))
        }
        linkDirectionalParticleWidth={(l) =>
          2 + Math.min(3, Math.log2((l as ZapLink).amount + 1) / 3)
        }
      ...
      <div className="absolute top-3 right-3 flex gap-2 pointer-events-auto">
        ...
        <RelayMultiSelect
          selected={relays}
          onChange={(r) => setSelectedRelays(r)}
        />
        <button
