...
const AUTHOR = normalizeToPubkey(
  ...
  return (
    <div className="flex flex-col gap-1 text-sm">
      <div className="flex items-center gap-2">
        ...
        {message && (
          <p className="ms-8 text-gray-600 whitespace-pre-wrap wrap-break-word">
            {message}
          </p>
        )}
        <span className="ms-auto text-warning font-bold whitespace-nowrap">
...
function NoteCard({ note }: { note: Note }) {
  ...
  const totalSats = useMemo(
    () =>
      zaps ? zaps.reduce((sum, z) => sum + Math.round(z.amount / 1000), 0) : 0,
    [zaps],
  );
...
export default function LoadingZaps() {
  ...
  const timeline = useMemo(() => {
    ...
    return createTimelineLoader(pool, relays, filter, {
      eventStore,
      limit: 50,
    });
  }, [filter, relays?.join("|")]);
  ...
  const notes = use$(
    () => eventStore.timeline(filter).pipe(castTimelineStream(Note)),
    [filter],
  );
    ...
    <div className="container mx-auto my-8 px-4 max-w-3xl">
      ...
      <div className="flex flex-col gap-4 py-4">
        ...
        <button
          ...
          onClick={() => timeline && timeline().subscribe()}
        >
          Load More
        </button>
      </div>
