...
function LinkRenderer({ node: link }: { node: Link }) {
  ...
  if (isAudioURL(link.href))
    return <audio src={link.href} className="rounded" controls />;
  return (
    <a
      href={link.href}
      target="_blank"
...
const components: ComponentMap = {
  ...
  emoji: ({ node }) => (
    <img
      title={node.raw}
      src={node.url}
      className="w-6 h-6 inline align-text-bottom"
    />
  ),
};
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2 items-center">
        ...
        <time className="ms-auto text-sm text-gray-500">
          {new Date(zap.event.created_at * 1000).toLocaleString()}
        </time>
      </div>

      {message && (
        <div className="whitespace-pre-wrap border-l-4 border-warning ps-3 py-1 text-sm">
          {renderedMessage}
        </div>
      )}
        ...
        <div className="card card-sm bg-base-200 shadow-md opacity-50">
          <div className="card-body">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-1">
                <p className="text-sm font-mono">
                  Loading event: {eventPointer!.id}
                </p>
                {eventPointer!.relays && eventPointer!.relays.length > 0 && (
                  <p className="text-xs text-gray-500">
                    Checking relays: {eventPointer!.relays.join(", ")}
                  </p>
                )}
              </div>
            </div>
...
export default function ZapsTimeline() {
  ...
  const zaps = use$(
    () =>
      eventStore
        .timeline({ kinds: [kinds.Zap] })
        .pipe(castTimelineStream(Zap, eventStore)),
    [],
  );
