...
function prepareChartData(zaps: Zap[], userPubkey: string, mode: ChartMode) {
  ...
  const items = zaps
    ...
    .filter(
      (z): z is { timestamp: number; amount: number; isIncoming: boolean } =>
        z !== null,
    )
    .sort((a, b) => a.timestamp - b.timestamp);

  // Group by day for better visualization
  const dailyZaps = items.reduce<
  ...
  >((acc, zap) => {
    ...
    if (zap.isIncoming)
      acc[date].incoming += mode === "amount" ? zap.amount : 1;
...
export default function ZapGraph() {
  ...
  const zaps = use$(
    () =>
      eventStore.timeline(filters).pipe(castTimelineStream(Zap, eventStore)),
    [filters],
  );
  ...
  const chartOptions = useMemo(
    () => ({
      ...
      plugins: {
        ...
        title: {
          text: `Zap Activity Over Time (${chartMode === "amount" ? "Amount" : "Count"})`,
        },
      },
      scales: {
        y: {
          ...
          min: 1,
          title: {
            text: chartMode === "amount" ? "Amount (sats)" : "Number of Zaps",
          },
        },
        x: { title: { text: "Date" } },
      },
      ...
      {pubkey ? (
        <div className="flex flex-col gap-4">
          ...
          <div className="text-center text-sm text-gray-500">
            Loaded {zaps?.length ?? 0} zaps
          </div>
        </div>
      ) : (
        <div className="alert alert-info">
          Please enter a pubkey to view zap activity
        </div>
      )}
    </div>
  );
