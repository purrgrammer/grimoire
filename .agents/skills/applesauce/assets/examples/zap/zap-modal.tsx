...
const storedWallet = safeParse<SerializedWalletConnect>(
...
wallet$.subscribe((wallet) => {
  if (wallet)
    localStorage.setItem("wallet-connect", JSON.stringify(wallet.toJSON()));
...
function LinkRenderer({ node: link }: { node: Link }) {
  if (isImageURL(link.href))
    ...
    return <video src={link.href} className="max-h-64 rounded mt-2" controls />;
  else if (isAudioURL(link.href))
    return <audio src={link.href} className="rounded mt-2" controls />;
  else
    return (
      <a
        href={link.href}
      ...
      <div className="avatar">
        <div className="w-6 rounded-full">
          <img
            src={
              profile?.picture ??
              `https://robohash.org/${zap.sender.pubkey}.png`
            }
          />
        </div>
      </div>
...
function NoteCard({ note, onZap }: { note: Note; onZap: () => void }) {
  ...
  const totalSats = useMemo(
    () =>
      zaps
        ? zaps.reduce((sum, z) => sum + Math.round((z.amount ?? 0) / 1000), 0)
        : 0,
    [zaps],
  );
...
async function fetchLNURLPayEndpoint(
  address: string,
...
function ConnectWalletStep({
  ...
  onConnect: (wallet: WalletConnect) => void;
  onBack: () => void;
}) {
  ...
  const nwcWallet = useMemo(
    () =>
      new WalletConnect({ pool, relays: [relay], secret: generateSecretKey() }),
    [relay],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm opacity-70">
        Scan the QR code with your NWC-compatible wallet, or paste a connection
        string below.
      </p>
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <RelayPicker
          value={relay}
          onChange={setRelay}
          common={["wss://relay.getalby.com/v1"]}
          className="w-full"
        />
      </div>
...
function ZapForm({ note, onClose }: { note: Note; onClose: () => void }) {
  ...
  const [lnurlEndpoint, setLnurlEndpoint] = useState<LNURLPayEndpoint | null>(
    null,
  );
    ...
    fetchLNURLPayEndpoint(addr)
      .then((endpoint) => {
        ...
        if (!endpoint.allowsNostr) {
          setLnurlError(
            "This user's lightning address does not support Nostr zaps (allowsNostr is false)",
          );
        }
      })
      .catch((err) => {
        setLnurlError(
          err instanceof Error
            ? err.message
            : "Failed to fetch lightning address info",
        );
      })
  ...
  return (
    <>
        ...
        <div className="space-y-4">
            ...
            <div className="relative text-xs opacity-40 h-4">
              {(
                ...
                <span
                  ...
                  style={{
                    left: `${(satsToSlider(sats) / SLIDER_STEPS) * 100}%`,
                  }}
                >
                  {label}
                </span>
          ...
          {lnurlEndpoint && (
            <p className="text-xs opacity-50">
              Min:{" "}
              {Math.ceil(lnurlEndpoint.minSendable / 1000).toLocaleString()}{" "}
              sats | Max:{" "}
              {Math.floor(lnurlEndpoint.maxSendable / 1000).toLocaleString()}{" "}
              sats
            </p>
          )}
                      ...
                      <div className="space-y-1 text-xs">
                        {parsed.amount && (
                          <p>
                            ...
                            {Math.round(parsed.amount / 1000).toLocaleString()}{" "}
                            sats
                          </p>
                        )}
                        <p>
      ...
      {step === "connect-wallet" && (
        <ConnectWalletStep
          onConnect={(w) => handleWalletPay(w)}
          onBack={() => setStep("awaiting-payment")}
        />
      )}
      ...
      {step === "paying-with-wallet" && (
        <div className="flex flex-col items-center gap-4 py-8">
          <span className="loading loading-spinner loading-lg text-warning"></span>
          <p className="text-sm opacity-70">
            Paying invoice with connected wallet...
          </p>
        </div>
      )}
      ...
      {step === "wallet-pay-error" && (
        <div className="flex flex-col gap-4 py-4">
          ...
          <p className="text-sm opacity-70 text-center">
            The connected wallet failed to pay the invoice. You can try
            connecting a different wallet or pay manually with the invoice QR
            code.
          </p>
...
export default function ZapModalExample() {
  ...
  const note = useMemo(
    () => (noteEvent ? castEvent(noteEvent, Note) : undefined),
    [noteEvent],
  );
  ...
  use$(() => {
    ...
    return pool
      .subscription(relays$, { kinds: [kinds.Zap], "#e": [note.id] })
      .pipe(
      ...
      {note ? (
        <>
          <NoteCard
            ...
            onZap={() =>
              (
                document.getElementById("zap_modal") as HTMLDialogElement
              )?.showModal()
            }
          />
          <ZapModal note={note} />
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs opacity-40 font-mono break-all max-w-md">
            {eventPointer.id}
          </p>
        </div>
      )}
